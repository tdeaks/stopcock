...
export const image = (href: string, w: number, h: number): Node => ({ kind: 'image', href, w, h, x: 0, y: 0 })
...
export const text = (value: string, size: number = 16): Node =>
  ({ kind: 'text', text: value, x: 0, y: 0, size })
