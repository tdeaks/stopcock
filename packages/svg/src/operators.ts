...
export const fill =
  (paint: Paint) =>
  (node: Node): Node =>
    withNode(node, { fill: paint })

export const stroke =
  (paint: Paint, width: number, opts: Omit<Stroke, 'paint' | 'width'> = {}) =>
  (node: Node): Node =>
    withNode(node, { stroke: { paint, width, ...opts } })

export const opacity =
  (value: number) =>
  (node: Node): Node =>
    withNode(node, { opacity: value })
...
export const rotate =
  (deg: number, cx: number = 0, cy: number = 0) =>
  (node: Node): Node => {
...
export const clip =
  (clipPath: ClipPath) =>
  (node: Node): Node =>
    withNode(node, { clip: clipPath })
export const mask =
  (maskValue: Mask) =>
  (node: Node): Node =>
    withNode(node, { mask: maskValue })
...
export const viewBox =
  ...
  (node: Node): Node => ({ kind: 'root', child: node, viewBox: [x, y, w, h] })

export const toClip =
  () =>
  (node: Node): ClipPath => ({ kind: 'clip', child: node })
export const toMask =
  () =>
  (node: Node): Mask => ({ kind: 'mask', child: node })
