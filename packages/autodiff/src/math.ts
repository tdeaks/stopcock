...
export const log = (x: ScalarInput): Var<number> => {
  const xv = asVar(x)
  return record(Math.log(xv.value), [xv], (grad) => [xv.value <= 0 ? Number.NaN : grad / xv.value])
}

export const sqrt = (x: ScalarInput): Var<number> => {
  const xv = asVar(x)
  const value = Math.sqrt(xv.value)
  return record(value, [xv], (grad) => [xv.value < 0 ? Number.NaN : grad / (2 * value)])
}
...
export const softplus = (x: ScalarInput): Var<number> => {
  const xv = asVar(x)
  const value =
    xv.value > 0 ? xv.value + Math.log1p(Math.exp(-xv.value)) : Math.log1p(Math.exp(xv.value))
