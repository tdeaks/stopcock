...
export function epochDaysToCivil(z: number): Civil {
  ...
  let yoe = Math.floor(
    (doe - Math.floor(doe / 1461) + Math.floor(doe / 36524) - Math.floor(doe / 146096)) / 365,
  ) // [0, 399]
...
export function timeComponents(ms: number): {
  hour: number
  minute: number
  second: number
  millisecond: number
...
export function compose(
  ...
  return (civilToEpochDays(year, month, day) * MS_DAY +
    hour * MS_HOUR +
    minute * MS_MINUTE +
    second * MS_SECOND +
    ms) as Timestamp
...
export function decompose(ts: Timestamp): {
  year: number
  month: number
  day: number
  hour: number
