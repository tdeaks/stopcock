...
// "Discriminator" group: N routes share identical segments at every position
// EXCEPT one literal position (the discriminator). Captures patterns like
// /api/v1/filler-{0..95}/:tenant/:id where the matcher would otherwise emit
// 96 individual structural-dispatch blocks and scan linearly. With this pass,
// the matcher walks a single prefix, slices the discriminator segment, and
// does one Map.get instead of N startsWith chains.
...
const groupByMidLiteralDiscriminator = (
  routes: RouteSpec[],
  indices: number[],
): { groups: DiscriminatorGroup[]; leftover: number[] } => {
  if (indices.length < 2) return { groups: [], leftover: [...indices] }
  // Skeleton groups routes by where params sit and what they're called. Two
  // routes are skeleton-compatible iff their segments match at every param
  // position (with the same name) — literal positions can differ.
  const bySkeleton = new Map<string, number[]>()
  for (const i of indices) {
    const segs = routes[i]!.path.split('/').filter(Boolean)
    const skel = segs.map((s) => s.startsWith(':') ? s : '*').join('/')
    const list = bySkeleton.get(skel) ?? []
    list.push(i)
    bySkeleton.set(skel, list)
  }
  const groups: DiscriminatorGroup[] = []
  const leftover: number[] = []
  for (const sameSkel of bySkeleton.values()) {
    if (sameSkel.length < 2) { leftover.push(...sameSkel); continue }
    const refSegsFirst = routes[sameSkel[0]!]!.path.split('/').filter(Boolean)
    const literalPositions: number[] = []
    for (let i = 0; i < refSegsFirst.length; i++) {
      if (!refSegsFirst[i]!.startsWith(':')) literalPositions.push(i)
    }
    // Search each literal position for the biggest sub-group that varies
    // only there. Greedy: take the largest such group; route any unclaimed
    // members to leftover.
    let best: { pos: number; indices: number[]; literals: string[] } | null = null
    for (const pos of literalPositions) {
      const byOther = new Map<string, number[]>()
      for (const i of sameSkel) {
        const segs = routes[i]!.path.split('/').filter(Boolean)
        const key = segs.map((s, j) => j === pos ? '?' : s).join('/')
        const list = byOther.get(key) ?? []
        list.push(i)
        byOther.set(key, list)
      }
      for (const sub of byOther.values()) {
        if (sub.length >= 2 && (!best || sub.length > best.indices.length)) {
          const literals = sub.map((i) => routes[i]!.path.split('/').filter(Boolean)[pos]!)
          best = { pos, indices: sub, literals }
        }
      }
    }
    if (best) {
      const refSegments = routes[best.indices[0]!]!.path.split('/').filter(Boolean)
      groups.push({ indices: best.indices, pos: best.pos, refSegments, literals: best.literals })
      const used = new Set(best.indices)
      for (const i of sameSkel) if (!used.has(i)) leftover.push(i)
    } else {
      leftover.push(...sameSkel)
    }
  }
...
export const generateMatcherSource = (routes: RouteSpec[]): CompilePlan => {
  ...
  const emitCell = (method: string, indices: number[], bucketKey: string, indent: string) => {
    ...
    // Mid-literal discriminator: routes sharing all but one literal segment.
    // Collapses N parallel structural dispatchers into one prefix-check +
    // Map.get on the varying segment. See groupByMidLiteralDiscriminator.
    const { groups: discGroups, leftover: afterDisc } = groupByMidLiteralDiscriminator(routes, leftover)
    discGroups.forEach((g, gi) => {
      const discMap = new Map<string, number>()
      g.literals.forEach((lit, k) => discMap.set(lit, g.indices[k]!))
      const discMapVar = `D_${method}_${bucketKey}_${gi}`
      closureData[discMapVar] = discMap

      const labelId = `_disc_${method}_${bucketKey}_${gi}`
      bodyLines.push(`${indent}// discriminator at pos ${g.pos}: /${g.refSegments.map((s, i) => i === g.pos ? '?' : s).join('/')}`)
      bodyLines.push(`${indent}${labelId}: {`)
      const ind = `${indent}  `

      // Prefix check: everything before the discriminator segment, with the
      // trailing '/' folded in so the discriminator slice starts on its first char.
      const prefixSegs = g.refSegments.slice(0, g.pos)
      let constPos = 0
      if (prefixSegs.length === 0) {
        bodyLines.push(`${ind}if (path.charCodeAt(0) !== 47) break ${labelId};`)
        constPos = 1
      } else {
        const prefix = '/' + prefixSegs.join('/') + '/'
        bodyLines.push(`${ind}if (!path.startsWith(${dq(prefix)}, 0)) break ${labelId};`)
        constPos = prefix.length
      }

      // Slice the discriminator segment.
      const isLastSeg = g.pos === g.refSegments.length - 1
      const dStartVar = `dS_${gi}`
      const dEndVar = `dE_${gi}`
      bodyLines.push(`${ind}var ${dStartVar} = ${constPos};`)
      if (isLastSeg) {
        bodyLines.push(`${ind}var ${dEndVar} = path.length;`)
      } else {
        bodyLines.push(`${ind}var ${dEndVar} = path.indexOf('/', ${dStartVar});`)
        bodyLines.push(`${ind}if (${dEndVar} === -1) break ${labelId};`)
      }
      bodyLines.push(`${ind}var disc_${gi} = path.slice(${dStartVar}, ${dEndVar});`)
      bodyLines.push(`${ind}idx = ${discMapVar}.get(disc_${gi});`)
      bodyLines.push(`${ind}if (idx === undefined) break ${labelId};`)

      // Trailing segments after the discriminator. Walk them with the same
      // structural logic the per-route block uses, capturing params.
      let varPos: string | null = dEndVar
      const paramVarOf: Record<string, string> = {}
      const trailingSegs = g.refSegments.slice(g.pos + 1)
      let slashConsumed = false
      for (let ti = 0; ti < trailingSegs.length; ti++) {
        const seg = trailingSegs[ti]!
        const isFinalSeg = ti === trailingSegs.length - 1
        if (seg.startsWith(':')) {
          const pname = seg.slice(1)
          if (!slashConsumed) {
            bodyLines.push(`${ind}if (path.charCodeAt(${varPos}) !== 47) break ${labelId};`)
          }
          const startVar = `s_${gi}_${ti}`
          const endVar = `e_${gi}_${ti}`
          bodyLines.push(`${ind}var ${startVar} = ${slashConsumed ? varPos : `(${varPos} + 1)`};`)
          slashConsumed = false
          if (isFinalSeg) {
            bodyLines.push(`${ind}var ${endVar} = path.length;`)
          ...
          const paramVar = `pv_${gi}_${ti}`
          bodyLines.push(`${ind}var ${paramVar} = path.slice(${startVar}, ${endVar});`)
          bodyLines.push(`${ind}if (${paramVar}.length === 0) break ${labelId};`)
          paramVarOf[pname] = paramVar
          varPos = endVar
        } else {
          // Literal trailing segment after discriminator. Rare for the cases
          // this pass targets (filler-N/:a/:b) but handle it for completeness.
          const nextIsParam = !isFinalSeg && trailingSegs[ti + 1]!.startsWith(':')
          const suffix = nextIsParam ? '/' : ''
          const piece = `/${seg}${suffix}`
          bodyLines.push(`${ind}if (!path.startsWith(${dq(piece)}, ${varPos})) break ${labelId};`)
          varPos = `(${varPos} + ${piece.length})`
          slashConsumed = nextIsParam
          if (isFinalSeg) {
            bodyLines.push(`${ind}if (${varPos} !== path.length) break ${labelId};`)
          }
        }
      }
      // All routes in the group share param names by construction; reuse the
      // first route's order for the params object.
      const refParamNames = g.refSegments.filter((s) => s.startsWith(':')).map((s) => s.slice(1))
      bodyLines.push(`${ind}SCRATCH.index = idx;`)
      if (refParamNames.length > 0) {
        const fields = refParamNames.map((n) => `${dq(n)}: ${paramVarOf[n]!}`).join(', ')
        bodyLines.push(`${ind}SCRATCH.params = { ${fields} };`)
      } else {
