...
describe('synth types', () => {
  ...
  it('keeps pipe transforms node-shaped', () => {
    ...
    expectTypeOf(oscillator.wavetable(createWavetable({ partials: [1, 0.5] }), 440)).toExtend<Node>()
    expectTypeOf(sampler.instrument({ zones: [{ samples: new Float32Array([0, 1]), rootMidi: 69 }] })).toExtend<Node>()
    expectTypeOf(instrument.lofiSampler({ zones: [{ samples: new Float32Array([0, 1]), rootMidi: 69 }], bits: 12, downsample: 2 })).toExtend<Node>()
    expectTypeOf(instrument.acidBass({ wave: 'square', resonance: 0.7, accent: 0.5 })).toExtend<Node>()
    ...
    expectTypeOf(fm({ freq: 110, operators: [{ kind: 'wavetable', bank: createWavetable({ partials: [1] }) }] })).toExtend<Node>()
    ...
    expectTypeOf(modulate(params.stringMachine.modulation, constant(1), 0.1)).returns.toExtend<Node>()
    ...
    expectTypeOf(effects.tapeDelay({ timeMs: 180, wow: 0.2, tone: 0.7, width: 0.8 })).returns.toExtend<Node>()
    expectTypeOf(effects.plateReverb({ preDelayMs: 12, decay: 0.6, damping: 0.4 })).returns.toExtend<Node>()
    expectTypeOf(effects.springReverb({ decay: 0.6, tension: 0.5, drip: 0.2 })).returns.toExtend<Node>()
    expectTypeOf(effects.nonlinearReverb({ timeMs: 160, decay: 0.7, drive: 0.3 })).returns.toExtend<Node>()
    expectTypeOf(effects.ensembleChorus({ rate: 0.4, depth: 4.44, width: 1 })).returns.toExtend<Node>()
    ...
    expectTypeOf(effects.wavefolder({ drive: 0.6, depth: 0.8, asymmetry: 0.2 })).returns.toExtend<Node>()
    ...
    expectTypeOf(effects.rotarySpeaker({ rate: 6.4, depth: 0.8, crossoverHz: 800 })).returns.toExtend<Node>()
    expectTypeOf(filter.stateVariable('bandpass', 1_200, { resonance: 0.7, drive: 0.2 })).returns.toExtend<Node>()
  })

  it('separates offline and web handle surfaces', () => {
    expectTypeOf(render(oscillator('sine', 440), { duration: 1 })).toEqualTypeOf<Float32Array | [Float32Array, Float32Array]>()
    expectTypeOf(voice.poly(input(0), { max: 4 }).play({} as AudioContext)).toExtend<WebAudioHandle>()
  })
