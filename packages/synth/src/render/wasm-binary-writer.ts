...
export class BinaryWriter {
  ...
  u32(value: number): void {
    if (!Number.isFinite(value) || value < 0 || value > 0xffff_ffff)
      throw new SynthCompileError('WASM binary graph value exceeds u32 range')
    this.ensure(4)
...
export function paramCode(param: AnyParam): number {
  ...
  if (op) {
    const offset = { ratio: 0, level: 1, feedback: 2, output: 3 }[
      op[2] as 'ratio' | 'level' | 'feedback' | 'output'
    ]
