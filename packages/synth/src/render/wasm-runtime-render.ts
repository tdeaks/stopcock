...
export function renderRuntimeWasm(
  node: Node,
  opts: RenderOptions,
  blockSize: number,
): Samples | null {
  ...
  const request = serializeWasmRuntimeRequestForWorklet(
    compiled.nodes,
    compiled.root,
    sampleRate,
    framesPerBlock,
  ...
  const leftPtr = directRuntime
    ? 0
    : wasm.stopcock_synth_alloc(framesPerBlock * Float32Array.BYTES_PER_ELEMENT)
  ...
  try {
    ...
    const inputHeap =
      inputPtr !== 0
        ? new Float32Array(wasm.memory.buffer, inputPtr, inputChannels * framesPerBlock)
        : undefined
    ...
    for (let offset = 0; offset < length; offset += framesPerBlock) {
      ...
      if (inputHeap && opts.inputs) {
        for (let channel = 0; channel < inputChannels; channel++) {
          inputHeap.set(
            opts.inputs[channel].subarray(offset, offset + frames),
            channel * framesPerBlock,
          )
        }
      }
    ...
    if (leftPtr !== 0)
      wasm.stopcock_synth_dealloc(leftPtr, framesPerBlock * Float32Array.BYTES_PER_ELEMENT)
    if (rightPtr !== 0)
      wasm.stopcock_synth_dealloc(rightPtr, framesPerBlock * Float32Array.BYTES_PER_ELEMENT)
    if (inputPtr !== 0)
      wasm.stopcock_synth_dealloc(
        inputPtr,
        inputChannels * framesPerBlock * Float32Array.BYTES_PER_ELEMENT,
      )
  }
}
