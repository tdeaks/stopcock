...
export function renderSingleWasm(wasm: WasmExports, node: Node, opts: RenderOptions, note?: Trigger): Samples | null {
  ...
  const request = binaryGraph
    ...
    : new TextEncoder().encode(JSON.stringify(serializeWasmJsonRenderRequest(node, opts, sampleRate, length, note)))
