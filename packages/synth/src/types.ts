...
export type SpaceEchoMode = 'head-1' | 'head-2' | 'head-3' | 'heads-1-2' | 'heads-2-3' | 'heads-1-3' | 'heads-1-2-3'
...
export type LofiSamplerParam = 'freq' | 'attack' | 'release' | 'level' | 'bits' | 'downsample' | 'jitter' | 'noise' | 'tone' | 'drive' | 'mix'
export type AcidBassParam = 'freq' | 'cutoff' | 'resonance' | 'envMod' | 'decay' | 'accent' | 'slide' | 'drive' | 'level'
export type DrumVoiceParam = 'freq' | 'decay' | 'tone' | 'snap' | 'noise' | 'drive' | 'level'
export type StringMachineParam = 'freq' | 'detune' | 'attack' | 'release' | 'tone' | 'depth' | 'modulation' | 'width' | 'level'
export type PolySynthParam = 'freq' | 'detune' | 'pulseWidth' | 'sub' | 'noise' | 'cutoff' | 'resonance' | 'envMod' | 'attack' | 'decay' | 'sustain' | 'release' | 'drive' | 'chorus' | 'modulation' | 'width' | 'level'
...
export type SpaceEchoParam = 'timeMs' | 'feedback' | 'mix' | 'reverbMix' | 'wow' | 'flutter' | 'tapeAge' | 'drive'
export type TapeDelayParam = 'timeMs' | 'feedback' | 'mix' | 'wow' | 'flutter' | 'tapeAge' | 'drive' | 'tone' | 'width'
export type PlateReverbParam = 'preDelayMs' | 'decay' | 'damping' | 'diffusion' | 'modulation' | 'mix' | 'width'
...
export type AnyParam =
  | OscParam | WavetableParam | ConstantParam | GainParam | PanParam | BiquadParam | StateVariableFilterParam | CombParam
  | SamplerInstrumentParam | LofiSamplerParam | AcidBassParam | DrumVoiceParam | StringMachineParam | PolySynthParam | AdsrParam | ArParam | ExponentialParam | DelayParam | ReverbParam
  | DistortionParam | ChorusParam | EnsembleChorusParam | SpaceEchoParam | TapeDelayParam | PlateReverbParam | SpringReverbParam | NonlinearReverbParam | MicroPitchParam | MultiTapDelayParam
  | SaturatorParam | WavefolderParam | DegradeParam | TiltEqParam | StereoSpreadParam | FrequencyShifterParam | RotarySpeakerParam | PhaserParam | CompressorParam | BitcrushParam | FmParam
...
export type FmOperatorInput = OperatorInputSource & Partial<Pick<FmOperator, 'ratio' | 'detune' | 'level' | 'feedback' | 'output' | 'phase'>>
...
export type Node = Common & (
  ...
  | { kind: 'wavetable', bank: WavetableBank, freq: Hz, detune: number, phase: number, position: number }
  | { kind: 'fm', freq: Hz, detune: number, index: number, operators: ReadonlyArray<FmOperator>, matrix: ReadonlyArray<ReadonlyArray<number>> }
  | { kind: 'noise', color: NoiseColor, seed: number }
  | { kind: 'constant', value: number }
  | { kind: 'buffer', samples: Float32Array, loop: boolean, rate: number }
  | { kind: 'samplerInstrument', zones: ReadonlyArray<SamplerZone>, freq: Hz, attack: number, release: number, level: number, velocity?: number }
  | { kind: 'lofiSampler', zones: ReadonlyArray<SamplerZone>, freq: Hz, attack: number, release: number, level: number, bits: number, downsample: number, jitter: number, noise: number, tone: number, drive: number, mix: number, velocity?: number }
  | { kind: 'acidBass', wave: AcidBassWaveform, freq: Hz, cutoff: Hz, resonance: number, envMod: number, decay: number, accent: number, slide: number, drive: number, level: number, velocity?: number }
  | { kind: 'drumVoice', drumKind: DrumVoiceKind, freq: Hz, decay: number, tone: number, snap: number, noise: number, drive: number, level: number, velocity?: number }
  | { kind: 'stringMachine', freq: Hz, detune: number, attack: number, release: number, tone: number, depth: number, modulation: number, width: number, level: number, velocity?: number }
  | { kind: 'polySynth', freq: Hz, detune: number, pulseWidth: number, sub: number, noise: number, cutoff: Hz, resonance: number, envMod: number, attack: number, decay: number, sustain: number, release: number, drive: number, chorus: number, modulation: number, width: number, level: number, velocity?: number }
  ...
  | { kind: 'stateVariableFilter', input: Node, mode: StateVariableFilterMode, freq: Hz, resonance: number, drive: number, mix: number }
  ...
  | { kind: 'ensembleChorus', input: Node, rate: Hz, depth: number, mix: number, width: number, tone: number, noise: number }
  | { kind: 'spaceEcho', input: Node, timeMs: number, feedback: number, mix: number, reverbMix: number, wow: number, flutter: number, tapeAge: number, drive: number, mode: SpaceEchoMode }
  | { kind: 'tapeDelay', input: Node, timeMs: number, feedback: number, mix: number, wow: number, flutter: number, tapeAge: number, drive: number, tone: number, width: number }
  | { kind: 'plateReverb', input: Node, preDelayMs: number, decay: number, damping: number, diffusion: number, modulation: number, mix: number, width: number }
  | { kind: 'springReverb', input: Node, decay: number, damping: number, tension: number, drip: number, mix: number, width: number }
  | { kind: 'nonlinearReverb', input: Node, timeMs: number, decay: number, damping: number, drive: number, mix: number, width: number }
  | { kind: 'microPitch', input: Node, detune: number, width: number, delayMs: number, mix: number }
  | { kind: 'multiTapDelay', input: Node, timeMs: number, feedback: number, mix: number, tone: number, width: number, taps: ReadonlyArray<Required<MultiTapDelayTap>> }
  | { kind: 'saturator', input: Node, drive: number, asymmetry: number, tone: number, mix: number, output: number }
  | { kind: 'wavefolder', input: Node, drive: number, depth: number, asymmetry: number, tone: number, mix: number, output: number }
  | { kind: 'degrade', input: Node, bits: number, downsample: number, jitter: number, noise: number, tone: number, mix: number }
  ...
  | { kind: 'rotarySpeaker', input: Node, rate: Hz, depth: number, mix: number, drive: number, width: number, freq: Hz }
