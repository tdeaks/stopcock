...
function boundedMap<A, B>(
  arr: readonly A[],
  fn: (a: A, signal?: AbortSignal) => Promise<B>,
  concurrency: number,
): Task<B[], unknown> {
...
function boundedFilter<A>(
  arr: readonly A[],
  pred: (a: A, signal?: AbortSignal) => Promise<boolean>,
  concurrency: number,
): Task<A[], unknown> {
  return of(async (signal?) => {
    if (arr.length === 0) return []
...
function boundedForEach<A>(
  arr: readonly A[],
  fn: (a: A, signal?: AbortSignal) => Promise<void>,
  concurrency: number,
): Task<void, unknown> {
  return of(async (signal?) => {
    if (arr.length === 0) return
...
export function forEachAsync<A>(
  arr: readonly A[],
  fn: (a: A, signal?: AbortSignal) => Promise<void>,
  concurrency?: number,
): Task<void, unknown>
export function forEachAsync<A>(
  fn: (a: A, signal?: AbortSignal) => Promise<void>,
  concurrency?: number,
): (arr: readonly A[]) => Task<void, unknown>
...
export function reduceAsync<A, B>(
  arr: readonly A[],
  fn: (acc: B, a: A, signal?: AbortSignal) => Promise<B>,
  init: B,
): Task<B, unknown>
export function reduceAsync<A, B>(
  fn: (acc: B, a: A, signal?: AbortSignal) => Promise<B>,
