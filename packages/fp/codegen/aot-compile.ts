...
const opNames: Record<number, string> = {
  ...
  [OP_TAKE]: 'take',
  [OP_DROP]: 'drop',
  [OP_TAKE_WHILE]: 'takeWhile',
  [OP_DROP_WHILE]: 'dropWhile',
  [OP_FLAT_MAP]: 'flatMap',
  [OP_REJECT]: 'reject',
  [OP_REDUCE]: 'reduce',
  [OP_FOR_EACH]: 'forEach',
...
function generateBody(
  ...
  const knownSize = pipe(
    streamOps,
    A.every((op: number) => op === OP_MAP),
  )
  ...
  const resultVar = usePrealloc
    ? 'var r=new Array(src.length)'
    : !hasTerminal && !hasAccessor
      ? 'var r=[]'
      : hasAccessor
        ? 'var r=[]'
        : lastOp === OP_REDUCE
  ...
  for (let i = 0; i < streamLen; i++) {
    switch (ops[i]) {
      ...
      case OP_DROP:
        ...
        break
      case OP_TAKE_WHILE:
        lines.push('if(!f' + i + '(v))break')
        break
      case OP_DROP_WHILE:
        lines.push('if(dw' + i + '){if(f' + i + '(v))continue;dw' + i + '=false}')
        break
      case OP_FLAT_MAP:
  ...
  if (hasAccessor || !hasTerminal) {
    ...
    switch (lastOp) {
      case OP_REDUCE:
        lines.push('acc=f' + ti + '(acc,v)')
        break
      case OP_FOR_EACH:
        lines.push('f' + ti + '(v)')
        break
      case OP_EVERY:
        lines.push('if(!f' + ti + '(v)){ev=false;break}')
        break
      case OP_SOME:
        lines.push('if(f' + ti + '(v)){sm=true;break}')
        break
      case OP_FIND:
        lines.push('if(f' + ti + '(v))return v')
        break
      case OP_FIND_INDEX:
        lines.push('if(f' + ti + '(v))return fi;fi++')
        break
      case OP_NONE:
        lines.push('if(f' + ti + '(v)){nn=true;break}')
        break
      case OP_COUNT:
        lines.push('if(f' + ti + '(v))cnt++')
        break
    }
  }
...
const TL = OP_TAIL,
  IN = OP_INIT,
  RV = OP_REVERSE
const UQ = OP_UNIQ_INLINE,
  SU = OP_SUM,
  MN = OP_MIN,
  MX = OP_MAX
const SA = OP_SORT_ASC,
  SD = OP_SORT_DESC
...
const diffCount = pipe(
  ...
  A.filter(
    (ops) =>
      generateBody(ops, ops.length, 'push').code !== generateBody(ops, ops.length, 'prealloc').code,
  ),
  (a) => a.length,
)

await Bun.write(new URL('../src/aot-compiled.ts', import.meta.url).pathname, unified)

console.log(
  `Generated ${patterns.length} AOT patterns (${diffCount} V8-branched) + ${chainExports.length} named chains → src/aot-compiled.ts`,
)
/// <reference types="bun" />
