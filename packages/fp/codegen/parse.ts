...
export const char = (c: string): Parser<string> => (input, pos) =>
  input[pos] === c
    ? { success: true, value: c, remaining: input.slice(pos + 1), position: pos + 1 }
    : { success: false, expected: `'${c}'`, position: pos }

export const string = (s: string): Parser<string> => (input, pos) =>
  input.startsWith(s, pos)
    ? { success: true, value: s, remaining: input.slice(pos + s.length), position: pos + s.length }
    : { success: false, expected: `'${s}'`, position: pos }

export const seq = (...parsers: Parser<any>[]): Parser<any[]> => (input, pos) => {
  const values: any[] = []
  let cursor = pos
...
export const map = <A, B>(parser: Parser<A>, fn: (a: A) => B): Parser<B> => (input, pos) => {
