...
// ReceiptSchemaV1 target: fp-fusion-debug
...
const hasHash = (value: unknown): value is string => typeof value === 'string' && HASH.test(value)

const hasId = (value: unknown): value is string => typeof value === 'string' && ID.test(value)
...
const hashes = (value: unknown, path: string, errors: string[], allowEmpty = true): void => {
  if (
    !Array.isArray(value) ||
...
const validatePlan = (value: Record<string, unknown>, errors: string[]): void => {
...
const validateRuntimeProfile = (value: Record<string, unknown>, errors: string[]): void => {
  ...
  for (const key of ['executions', 'consumedItems', 'hotness', 'timingNanoseconds'] as const) {
  ...
  if (!isRecord(value.privacy)) {
    ...
    exactKeys(value.privacy, ['inputValues', 'callbackCaptures', 'resultValues'], 'privacy', errors)
...
const validateEvidenceRef = (value: Record<string, unknown>, errors: string[]): void => {
