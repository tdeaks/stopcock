 ...
 *
 * `interpret` below is the one generic exact executor compact runs. A
 * separately written compact executor would be a second place for early-exit
 * and sink semantics to drift, and compact's job is to be small and exactly
 * right, not to be fast. Optimized fusion remains where speed lives. The
 * sequential tier (`internal/sequential.ts`, real ops applied one at a time)
 * is the differential oracle tests check this executor against; `interpret`
 * itself is production code, not a reference implementation.
 *
