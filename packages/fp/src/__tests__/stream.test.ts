...
describe('transformers', () => {
  ...
  it('takeWhile', () => {
    expect(
      pipe(
        Stream.from([1, 2, 3, 4, 1]),
        Stream.takeWhile((x) => x < 3),
        Stream.toArray,
      ),
    ).toEqual([1, 2])
  })
  ...
  it('dropWhile', () => {
    expect(
      pipe(
        Stream.from([1, 2, 3, 2, 1]),
        Stream.dropWhile((x) => x < 3),
        Stream.toArray,
      ),
    ).toEqual([3, 2, 1])
  })
  ...
  it('chunk', () => {
    expect(pipe(Stream.range(0, 7), Stream.chunk(3), Stream.toArray)).toEqual([
      [0, 1, 2],
      [3, 4, 5],
      [6],
    ])
  ...
  it('intersperse', () => {
    expect(pipe(Stream.from([1, 2, 3]), Stream.intersperse(0), Stream.toArray)).toEqual([
      1, 0, 2, 0, 3,
    ])
  })
})
...
describe('terminals', () => {
  it('reduce', () => {
    expect(
      pipe(
        Stream.from([1, 2, 3, 4]),
        Stream.reduce((a: number, b: number) => a + b, 0),
      ),
    ).toBe(10)
  })
...
describe('distinctN', () => {
  it('clears set after reaching cap', () => {
    ...
    const result = pipe(Stream.from([1, 2, 3, 1, 2, 3]), distinctN(2), Stream.toArray)
  ...
  it('acts like distinct when maxSize is large', () => {
    const result = pipe(Stream.from([1, 2, 2, 3, 1]), distinctN(100), Stream.toArray)
    expect(result).toEqual([1, 2, 3])
  })

  it('works as data-last', () => {
    const result = pipe(Stream.from([1, 1, 2, 2, 3]), Stream.distinctN(50), Stream.toArray)
    expect(result).toEqual([1, 2, 3])
  })
})
...
describe('zip edge cases', () => {
  ...
  it('terminates at shorter stream (right shorter)', () => {
    const result = pipe(Stream.from([1, 2, 3, 4]), Stream.zip(Stream.from(['a'])), Stream.toArray)
    expect(result).toEqual([[1, 'a']])
  })

  it('empty + non-empty = empty', () => {
    const result = pipe(Stream.empty<number>(), Stream.zip(Stream.from([1, 2])), Stream.toArray)
    expect(result).toEqual([])
  })
})
