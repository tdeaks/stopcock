...
test('pipe rejects calls past the public overload limit', () => {
  ...
  pipe(0, step, step, step, step, step, step, step, step, step, step, step, step, step, step, step, step, step, step, step, step, step)
})
...
test('flow rejects calls past the public overload limit', () => {
  ...
  flow(step, step, step, step, step, step, step, step, step, step, step, step, step, step, step, step, step, step, step, step, step)
})
...
test('Array data-last functions infer input and output values', () => {
  ...
  expectTypeOf(A.map((n: number) => String(n))).toEqualTypeOf<(arr: readonly number[]) => string[]>()
  expectTypeOf(A.mapWithIndex((n: number, i) => `${i}:${n}`)).toEqualTypeOf<(arr: readonly number[]) => string[]>()
  expectTypeOf(A.filter((n: number) => n > 1)).toEqualTypeOf<(arr: readonly number[]) => number[]>()
  expectTypeOf(A.filterWithIndex((n: number, i) => n > i)).toEqualTypeOf<(arr: readonly number[]) => number[]>()
  expectTypeOf(A.flatMap((n: number) => [String(n)])).toEqualTypeOf<(arr: readonly number[]) => string[]>()
  expectTypeOf(A.filterMap((n: number) => n > 1 ? String(n) : undefined)).toEqualTypeOf<(arr: readonly number[]) => string[]>()
  expectTypeOf(A.findMap((n: number) => n > 1 ? String(n) : undefined)).toEqualTypeOf<(arr: readonly number[]) => string | undefined>()
  expectTypeOf(A.mapWhile((n: number) => n < 3 ? String(n) : undefined)).toEqualTypeOf<(arr: readonly number[]) => string[]>()
  expectTypeOf(A.takeUntil((n: number) => n > 2)).toEqualTypeOf<(arr: readonly number[]) => number[]>()
  expectTypeOf(A.find((n: number) => n > 1)).toEqualTypeOf<(arr: readonly number[]) => number | undefined>()
  expectTypeOf(A.findIndex((n: number) => n > 1)).toEqualTypeOf<(arr: readonly number[]) => number | undefined>()
  ...
  expectTypeOf(A.partition((n: number) => n > 0)).toEqualTypeOf<(arr: readonly number[]) => [number[], number[]]>()
  expectTypeOf(A.reject((n: number) => n > 0)).toEqualTypeOf<(arr: readonly number[]) => number[]>()
  expectTypeOf(A.reduce((total: number, n: number) => total + n, 0)).toEqualTypeOf<(arr: readonly number[]) => number>()
  ...
  expectTypeOf(A.zipWith(['a'], (n: number, s) => `${s}:${n}`)).toEqualTypeOf<(a: readonly number[]) => string[]>()
  // Current gap: pluck does not infer B from the requested key.
  expectTypeOf(A.pluck('id')).toEqualTypeOf<<A, B>(arr: readonly A[]) => B[]>()
  expectTypeOf(A.sortBy((a: { id: number }, b) => a.id - b.id)).toEqualTypeOf<(arr: readonly { id: number }[]) => { id: number }[]>()
  expectTypeOf(A.uniqBy((value: { id: number }) => value.id)).toEqualTypeOf<(arr: readonly { id: number }[]) => { id: number }[]>()
  expectTypeOf(A.groupBy((value: { kind: string }) => value.kind)).toEqualTypeOf<(arr: readonly { kind: string }[]) => Record<string, { kind: string }[]>>()
...
test('Object data-last functions infer object changes and path values', () => {
  ...
  expectTypeOf(Obj.assoc('role', 'admin')).toEqualTypeOf<(obj: User) => User & Record<string, string>>()
  expectTypeOf(Obj.mergeDeepLeft({ extra: true })).toEqualTypeOf<(a: User) => User & { extra: boolean }>()
  expectTypeOf(Obj.mergeDeepRight({ extra: true })).toEqualTypeOf<(a: User) => User & { extra: boolean }>()
  expectTypeOf(Obj.mergeWith({ id: 'fallback', name: 'Grace', meta: { active: false, score: 0 } }, (l: string, r) => l || r)).toEqualTypeOf<(a: User) => User>()
  ...
  expectTypeOf(Obj.path(tupleActivePath)).toEqualTypeOf<(obj: OptionalUser) => boolean | undefined>()
  ...
  expectTypeOf(Obj.evolve<User>({ name: name => name.trim() })).toEqualTypeOf<(obj: User) => User>()
})
...
test('Option preserves value inference through constructors, transforms, and extraction', () => {
  ...
  const mapped = pipe(value, O.map(n => String(n)))
  const flatMapped = pipe(value, O.flatMap(n => O.some(String(n))))
  ...
  expectTypeOf(pipe(value, O.getOrElse(() => 'fallback'))).toEqualTypeOf<number | string>()
...
test('Option completeness combinators preserve data-last inference', () => {
  ...
  expectTypeOf(pipe(value, O.orElseWith(() => fallback))).toEqualTypeOf<O.Option<number | string>>()
  expectTypeOf(pipe(value, O.and(fallback))).toEqualTypeOf<O.Option<string>>()
  expectTypeOf(pipe(value, O.andThen(n => O.some(String(n))))).toEqualTypeOf<O.Option<string>>()
  ...
  expectTypeOf(pipe(value, O.zipWith(O.some('label'), (n, label) => `${label}:${n}`))).toEqualTypeOf<O.Option<string>>()
  ...
  expectTypeOf(pipe(value, O.exists(n => n > 0))).toEqualTypeOf<boolean>()
  expectTypeOf(pipe(value, O.mapNullable(n => n > 0 ? String(n) : null))).toEqualTypeOf<O.Option<string>>()
})

test('Result preserves value and error inference through transforms and extraction', () => {
  const value = R.ok(1)
  const error = R.err({ code: 'bad' as const })
  const mapped = pipe(value, R.map(n => String(n)))
  const mappedErr = pipe(error, R.mapErr(e => e.code))
  const flatMapped = pipe(value, R.flatMap(n => n > 0 ? R.ok(String(n)) : R.err('negative' as const)))
  ...
  expectTypeOf(pipe(error, R.getOrElse(() => 0))).toEqualTypeOf<number>()
...
test('Result completeness combinators preserve data-last inference', () => {
  ...
  expectTypeOf(pipe(error, R.orElseWith(e => R.err(e.code)))).toEqualTypeOf<R.Result<never, 'bad'>>()
  expectTypeOf(pipe(value, R.and(fallback))).toEqualTypeOf<R.Result<string, never>>()
  expectTypeOf(pipe(value, R.andThen(n => R.ok(String(n))))).toEqualTypeOf<R.Result<string, never>>()
  ...
  expectTypeOf(pipe(value, R.zipWith(R.ok('label'), (n, label) => `${label}:${n}`))).toEqualTypeOf<R.Result<string, never>>()
  expectTypeOf(pipe(value, R.contains(1))).toEqualTypeOf<boolean>()
  expectTypeOf(pipe(R.ok('a' as 'a' | 'b') as R.Result<'a' | 'b', Error>, R.contains('a'))).toEqualTypeOf<boolean>()
  expectTypeOf(R.contains('a')(unionValue)).toEqualTypeOf<boolean>()
  expectTypeOf(R.contains<'a'>('a')(unionValue)).toEqualTypeOf<boolean>()
  expectTypeOf(pipe(value, R.exists(n => n > 0))).toEqualTypeOf<boolean>()
...
test('Guard exports predicate, refinement, and brand types', () => {
  ...
  expectTypeOf<G.Refinement<string | number, string>>().toEqualTypeOf<(a: string | number) => a is string>()
...
test('Guard refinement combinators preserve narrowing', () => {
  ...
  expectTypeOf(G.and(isActive, hasName)).toEqualTypeOf<G.Refinement<RawAccount, ActiveAccount & NamedAccount>>()
  expectTypeOf(G.or(isUser, isGuest)).toEqualTypeOf<G.Refinement<Account, User | Guest>>()
  expectTypeOf(G.not(isStringValue)).toEqualTypeOf<G.Refinement<string | number | boolean, number | boolean>>()
})
