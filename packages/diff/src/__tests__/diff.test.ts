...
describe('diff', () => {
  ...
  it('handles arrays', () => {
    const p = diff([1, 2, 3, 4])([1, 2, 3])
    expect(p.ops.length).toBeGreaterThan(0)
    expect(applyUnsafe(p)([1, 2, 3])).toEqual([1, 2, 3, 4])
  })

  it('handles array deletions', () => {
    const p = diff([1, 3])([1, 2, 3])
    expect(applyUnsafe(p)([1, 2, 3])).toEqual([1, 3])
  })
  ...
  it('handles non-empty to empty array', () => {
    const p = diff([])([1, 2])
    expect(applyUnsafe(p)([1, 2])).toEqual([])
  })

  it('handles type changes', () => {
    const p = diff({ a: 42 })({ a: 'string' })
    expect(p.ops[0].op).toBe('replace')
  })
  ...
  it('detects moves in arrays', () => {
    ...
    const p = diff(b)(a)
    expect(applyUnsafe(p)(a)).toEqual(b)
  })
  ...
  it('handles mixed array/object to different type', () => {
    const p = diff({ a: { x: 1 } })({ a: [1] })
    expect(p.ops[0].op).toBe('replace')
    expect(p.ops[0].path).toEqual(['a'])
  })

  it('handles null values', () => {
    const p = diff({ a: 1 })({ a: null })
    expect(p.ops).toHaveLength(1)
  })
