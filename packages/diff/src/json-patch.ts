...
function opToJson(op: Operation): JsonPatchOperation {
  switch (op.op) {
    case 'add':
      return { op: 'add', path: toPointer(op.path), value: op.value }
    case 'remove':
      return { op: 'remove', path: toPointer(op.path) }
    case 'replace':
      return { op: 'replace', path: toPointer(op.path), value: op.newValue }
    case 'move':
      return { op: 'move', path: toPointer(op.path), from: toPointer(op.from) }
    case 'test':
      return { op: 'test', path: toPointer(op.path), value: op.value }
    case 'rename':
      return {
        op: 'move',
...
function jsonToOp(op: JsonPatchOperation): Operation {
  ...
  switch (op.op) {
    case 'add':
      return { op: 'add', path, value: op.value }
    case 'remove':
      return { op: 'remove', path, oldValue: undefined }
    case 'replace':
      return { op: 'replace', path, oldValue: undefined, newValue: op.value }
    case 'move':
      return { op: 'move', from: fromPointer(op.from), path }
    case 'test':
      return { op: 'test', path, value: op.value }
    case 'copy':
      return { op: 'add', path, value: op.value }
  }
}
