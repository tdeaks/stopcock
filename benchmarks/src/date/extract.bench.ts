...
describe.each([100, 1_000, 10_000])('getYear (batch) — n=%i', (n) => {
  ...
  bench('stopcock', () => {
    for (let i = 0; i < ts.length; i++) D.getYear(ts[i]!)
  })
  bench('date-fns', () => {
    for (let i = 0; i < ds.length; i++) dfnsGetYear(ds[i]!)
  })
  bench('luxon', () => {
    for (let i = 0; i < ts.length; i++) DateTime.fromMillis(ts[i]! as number).year
  })
  bench('moment', () => {
    for (let i = 0; i < ts.length; i++) moment(ts[i]! as number).year()
  })
})
...
describe.each([100, 1_000, 10_000])('getWeekday (batch) — n=%i', (n) => {
  ...
  bench('stopcock', () => {
    for (let i = 0; i < ts.length; i++) D.getWeekday(ts[i]!)
  })
  bench('date-fns', () => {
    for (let i = 0; i < ds.length; i++) dfnsGetDay(ds[i]!)
  })
  bench('luxon', () => {
    for (let i = 0; i < ts.length; i++) DateTime.fromMillis(ts[i]! as number).weekday
  })
  bench('moment', () => {
    for (let i = 0; i < ts.length; i++) moment(ts[i]! as number).day()
  })
})
