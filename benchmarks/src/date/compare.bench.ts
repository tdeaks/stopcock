...
describe.each([100, 1_000, 10_000])('isBefore (batch) — n=%i', (n) => {
  ...
  bench('stopcock', () => {
    for (let i = 0; i < pairs.length; i++) pairs[i]![0] < pairs[i]![1]
  })
  bench('date-fns', () => {
    for (let i = 0; i < datePairs.length; i++) dfnsIsBefore(datePairs[i]![0], datePairs[i]![1])
  })
  bench('luxon', () => {
    for (let i = 0; i < pairs.length; i++)
      DateTime.fromMillis(pairs[i]![0] as number) < DateTime.fromMillis(pairs[i]![1] as number)
  })
  bench('moment', () => {
    for (let i = 0; i < pairs.length; i++)
      moment(pairs[i]![0] as number).isBefore(moment(pairs[i]![1] as number))
  })
})
