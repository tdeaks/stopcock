...
describe('deep 10-step filter+map→take(50) — n=10M', () => {
  ...
  bench('lodash', () =>
    _.flow([
      (d: number[]) => _.filter(d, gt50),
      (d: number[]) => _.map(d, dbl),
      (d: number[]) => _.filter(d, gt80),
      (d: number[]) => _.map(d, inc),
      (d: number[]) => _.filter(d, lt5),
      (d: number[]) => _.map(d, times10),
      (d: number[]) => _.filter(d, gt10),
      (d: number[]) => _.map(d, round),
      (d: number[]) => _.filter(d, isEven),
      (d: number[]) => _.take(d, 50),
    ])(data))
  bench('native', () =>
    data
      .filter(gt50)
