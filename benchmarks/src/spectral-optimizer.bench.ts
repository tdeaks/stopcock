...
import {
  buildPatch,
  renderPatchAudio,
  triggerForVerification,
} from '../../apps/docs/src/lib/spectral-optimizer-synth'
...
const landscapeFreqs = new Float32Array(
  Array.from({ length: landscapeBinCount }, (_, i) => {
...
describe('spectral optimizer showcase', () => {
  ...
  bench('verification synth core render', () =>
    renderSynth(patch, { sampleRate: 24_000, duration: 2.05, triggers: verificationTrigger }))
  ...
  bench('landscape SVG composition', () =>
    makeLandscape(
      landscapeGrid(),
      history.map((frame) => frame.params),
      palette,
      { cols: 48, rows: 48 },
    ))
})
