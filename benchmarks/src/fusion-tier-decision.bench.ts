 ...
 * pipeline shapes? Four executors per shape:
 *
 ...
 *
 * Startup: all four are asserted identical per shape/size before any bench
 * runs. Early-exit shapes also count callback invocations per executor,
 * printed once via console.table at collection time -- fused/naive counts
 * legitimately differ (that's the point); only output identity is a bar.
 */
...
function recordCounts(shape: string, n: number, counts: Record<string, number>): void {
  for (const executor of ['fused', 'naive', 'hand', 'compiled']) {
...
function findCounts(xs: readonly number[]): Record<string, number> {
  ...
  return { fused: fusedN, naive: naiveN, hand: handN, compiled: c.n }
}
