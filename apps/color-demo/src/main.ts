...
const h = <T extends HTMLElement = HTMLElement>(
  tag: string,
...
const setBase = (c: Color) => {
  base = c
  for (const s of subscribers) s()
}
const setLens = (l: CVDType | 'none') => {
  lens = l
  for (const s of subscribers) s()
}
...
const sectionInput = () => {
  ...
  return h(
    'section',
    { className: 'card', 'aria-labelledby': pickerId },
    h(
      'div',
      { className: 'row' },
      h('div', { className: 'col' }, h('label', { for: pickerId }, 'Pick'), picker),
      h('div', { className: 'col' }, h('label', { for: textId }, 'CSS color string'), text),
    ),
  )
}
...
const sectionLens = () => {
  ...
  const group = h('div', {
    ...
    role: 'radiogroup',
    'aria-label': 'Color vision deficiency lens',
  })
  for (const opt of options) {
    ...
    const input = h<HTMLInputElement>('input', {
      ...
      id,
      value: opt.value,
      checked: opt.value === lens ? '' : undefined,
    })
    if (opt.value === lens) input.checked = true
    input.addEventListener('change', () => {
      if (input.checked) setLens(opt.value)
    })
  ...
  return h(
    ...
    h(
      'p',
      { className: 'palette-label', style: { marginTop: '0', marginBottom: '10px' } },
...
const SPACES: { name: string; render: (c: Color) => string }[] = [
  ...
  {
    name: 'linear-srgb',
    render: (c) => fmtFn('color(srgb-linear', convert(c, 'linear-srgb'), 4, ')'),
  },
]
...
const sectionLightnessScale = () => {
  const oklchRow = h('div', {
    className: 'swatches',
    role: 'list',
    'aria-label': 'OKLCh lightness scale',
  })
  const hslRow = h('div', {
    className: 'swatches',
    role: 'list',
    'aria-label': 'HSL lightness scale',
  })
  ...
  const refresh = () => {
    ...
    for (let i = 1; i <= 9; i++) {
      ...
      const okStep: Color = {
        space: 'oklch',
        channels: new Float64Array([t, ok.channels[1], ok.channels[2]]),
        alpha: 1,
      }
      const hslStep: Color = {
        space: 'hsl',
        channels: new Float64Array([hsl.channels[0], hsl.channels[1], t]),
        alpha: 1,
      }
  ...
  return h(
    ...
    h(
      'div',
      { className: 'palette' },
      h('div', { className: 'palette-label' }, 'OKLCh L stepped'),
      oklchRow,
    ),
    h(
      'div',
      { className: 'palette' },
      h('div', { className: 'palette-label' }, 'HSL L stepped'),
      hslRow,
    ),
  )
}
...
const sectionPalettes = () => {
  ...
  const renderRow = (label: string, palette: Color[]) =>
    h(
      ...
      h(
        'div',
        { className: 'swatches', role: 'list', 'aria-label': label },
...
const sectionMix = () => {
  ...
  const targetPicker = h<HTMLInputElement>('input', {
    type: 'color',
    id: targetId,
    value: '#facc15',
    'aria-label': 'Mix target color',
  })
  const STEPS = 13
    ...
    container.replaceChildren(
      h(
        ...
        h(
          ...
          h('div', {
            className: 'swatch',
            'aria-label': `Color A: ${toHex(base)}`,
            style: { background: bg(base), width: '56px', height: '56px', borderRadius: '10px' },
          }),
        ),
      ...
      h(
        ...
        h('label', {}, 'sRGB'),
        buildGradient('srgb', 'Gradient mixed in sRGB'),
      ),
      h(
        'div',
        { className: 'gradient-row' },
        h('label', {}, 'HSL'),
        buildGradient('hsl', 'Gradient mixed in HSL'),
      ),
      h(
        'div',
        { className: 'gradient-row' },
        h('label', {}, 'OKLab'),
        buildGradient('oklab', 'Gradient mixed in OKLab'),
      ),
      h(
        'div',
        { className: 'gradient-row' },
        h('label', {}, 'OKLCh'),
        buildGradient('oklch', 'Gradient mixed in OKLCh'),
      ),
    )
  }
...
const sectionContrast = () => {
  ...
  const partnerPicker = h<HTMLInputElement>('input', {
    type: 'color',
    id: partnerId,
    value: '#ffffff',
  ...
  const badges = h('div', {
    className: 'badges',
    role: 'list',
    'aria-label': 'WCAG contrast results',
  })
  ...
  const refresh = () => {
    ...
    badges.replaceChildren(
      ...checks.map(([label, ok]) =>
        h(
          'span',
          { className: `badge ${ok ? 'pass' : 'fail'}`, role: 'listitem' },
          `${ok ? '✓' : '✗'} ${label}`,
        ),
      ),
    )
  }
  partnerPicker.addEventListener('input', () => {
    contrastPartner = fromHex(partnerPicker.value)
    refresh()
  })
  onChange(refresh)
...
const sectionContrastMatrix = () => {
  ...
  const refresh = () => {
    ...
    container.replaceChildren(
      h(
        'p',
        { className: 'palette-label', style: { marginTop: 0 } },
...
const sectionGamut = () => {
  ...
  const refresh = () => {
    ...
    const wide: Color = {
      space: 'oklch',
      channels: new Float64Array([ok.channels[0], C, ok.channels[2]]),
      alpha: 1,
    }
    ...
    cellNaive.replaceChildren(
      h('span', { className: 'label' }, `naive ${within ? '· in gamut' : '· clipped'}`),
    )
  ...
  return h(
    ...
    h(
      'div',
      { className: 'row', style: { marginBottom: '16px' } },
      h('label', { for: chromaId }, 'OKLCh chroma:'),
      chromaSlider,
      chromaValue,
    ),
...
app.append(
  ...
  h(
    'p',
    { className: 'subtitle' },
    'Every section reacts live to the base color. Use the CVD lens to preview how your palette appears to viewers with color vision differences.',
  ),
  ...
  h(
    'footer',
    {},
    '@stopcock/color — built per docs/color-plan.md · pure TypeScript · 0 deps beyond @stopcock/fp',
  ),
)
