...
const WHITE_KEYS: ReadonlyArray<WhiteKey> = [
  { midi: 48, label: 'C3' }, { midi: 50, label: 'D3' }, { midi: 52, label: 'E3' },
  { midi: 53, label: 'F3' }, { midi: 55, label: 'G3' }, { midi: 57, label: 'A3' },
  { midi: 59, label: 'B3' }, { midi: 60, label: 'C4' }, { midi: 62, label: 'D4' },
  { midi: 64, label: 'E4' }, { midi: 65, label: 'F4' }, { midi: 67, label: 'G4' },
  { midi: 69, label: 'A4' }, { midi: 71, label: 'B4' }, { midi: 72, label: 'C5' },
]
    ...
    <section class="keyboard-wrap">
      <div class="keyboard-header">
        ...
        <div class="keyboard-info">
          <span>OCTAVE<strong>C3 – C5</strong></span>
          <span>QWERTY<strong>A S D F G H J K · W E T Y U</strong></span>
          <span>MIDI<strong>{props.midiStatus}</strong></span>
          <span>VOICES<strong>poly · 8 · {activeNotes().size} held</strong></span>
        </div>
      </div>
      <div class="keyboard">
        <For each={WHITE_KEYS}>{(k) => (
          <div
            class={keyClass('key-white', k.midi)}
            data-label={k.label.startsWith('C') ? k.label : ''}
            onMouseDown={(e) => { e.preventDefault(); noteOn(k.midi) }}
