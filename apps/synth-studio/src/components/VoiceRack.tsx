...
import type {
  EnvEngine, FltEngine, FltMode, LfoEngine, OscEngine,
} from '../engine'
...
export const VoiceRack: Component = () => {
  ...
  return (
    <section class="rack" aria-label="Voice rack">
      ...
      >
        <WaveButtons current={state.osc.wave} onSelect={(w) => setState('osc', 'wave', w as OscWave)} />
        <div class="knob-row cols-4">
          <Knob label="TUNE"   value={state.osc.tune}        min={-24} max={24}  unit="st" onChange={(v) => setState('osc', 'tune', v)} />
          <Knob label="FINE"   value={state.osc.fine}        min={-100} max={100} unit="¢" onChange={(v) => setState('osc', 'fine', v)} />
          <Knob label="LEVEL"  value={state.osc.level * 100} min={0}   max={100} unit="%" onChange={(v) => setState('osc', 'level', v / 100)} />
          <Knob label="UNISON" value={state.osc.detune * 100} min={0}   max={100}           onChange={(v) => setState('osc', 'detune', v / 100)} />
        </div>
      </Module>
      ...
      >
        ...
        />
        <div class="knob-row cols-4">
          <Knob label="CUTOFF" value={state.flt.cutoff}     min={40} max={14000} unit="Hz" log onChange={(v) => setState('flt', 'cutoff', v)} />
          <Knob label="RES"    value={state.flt.res * 100}  min={0}  max={100}  unit="%"     onChange={(v) => setState('flt', 'res', v / 100)} />
          <Knob label="DRIVE"  value={state.flt.drive * 100} min={0}  max={100}  unit="%"     onChange={(v) => setState('flt', 'drive', v / 100)} />
          <Knob label="KEY"    value={state.flt.key * 100}  min={0}  max={100}  unit="%"     onChange={(v) => setState('flt', 'key', v / 100)} />
        </div>
      </Module>
      ...
      >
        ...
        />
        <div class="knob-row cols-4">
          <Knob label="ATK" value={state.env.atk * 1000} min={0} max={2000} unit="ms" log onChange={(v) => setState('env', 'atk', v / 1000)} />
          <Knob label="DEC" value={state.env.dec * 1000} min={0} max={2000} unit="ms" log onChange={(v) => setState('env', 'dec', v / 1000)} />
          <Knob label="SUS" value={state.env.sus * 100}  min={0} max={100}  unit="%"      onChange={(v) => setState('env', 'sus', v / 100)} />
          <Knob label="REL" value={state.env.rel * 1000} min={0} max={3000} unit="ms" log onChange={(v) => setState('env', 'rel', v / 1000)} />
        </div>
      </Module>
      ...
      >
        ...
        <div class="knob-row cols-3">
          <Knob label="RATE"  value={state.lfo.rate}        min={0.05} max={20} unit="Hz" log onChange={(v) => setState('lfo', 'rate', v)} />
          <Knob label="DEPTH" value={state.lfo.depth * 100} min={0}    max={100} unit="%"     onChange={(v) => setState('lfo', 'depth', v / 100)} />
          <Knob label="PHASE" value={state.lfo.phase}       min={0}    max={360} unit="°"     onChange={(v) => setState('lfo', 'phase', v)} />
        </div>
      </Module>
    </section>
  )
