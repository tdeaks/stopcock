...
export const PerfStrip: Component = () => {
  ...
  onCleanup(() => { if (timer !== undefined) window.clearInterval(timer) })
  ...
  const underrunsClass = createMemo(() => underrunsNow() > 0 ? 'perf-cell perf-warn' : 'perf-cell')

  return (
    <div class="perf-strip" aria-label="Engine telemetry">
      <span class="perf-label">Engine</span>
      <span class="perf-cell"><em>state</em><b>{engine() ? 'running' : 'idle'}</b></span>
      <span class="perf-cell"><em>sample</em><b>{sampleRateKHz()} kHz</b></span>
      <span class="perf-cell"><em>buffer</em><b>{bufferSize()} fr</b></span>
      <span class="perf-cell"><em>latency</em><b>{latencyMs()} ms</b></span>
      <span class="perf-cell"><em>voices</em><b>{voicesNow()} / 8</b></span>
      <span class="perf-cell"><em>fx</em><b>{activeFx()} / 4</b></span>
      <span class="perf-cell"><em>frame</em><b>{frameMs() > 0 ? frameMs().toFixed(1) : '—'} ms</b></span>
      <span class="perf-cell"><em>fps</em><b>{fps() || '—'}</b></span>
      <span class={underrunsClass()}><em>underrun</em><b>{underrunsNow()}</b></span>
      <span class="perf-cell"><em>uptime</em><b>{uptime()}</b></span>
    </div>
  )
}
