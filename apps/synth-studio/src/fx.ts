...
export const fxCatalog: Record<FxKind, FxDef> = {
  ...
  drive: {
    ...
    tag: 'DRV',
    category: 'distortion',
    params: [{ id: 'amount', label: 'AMOUNT', min: 0, max: 0.95, value: 0.3, unit: '%' }],
  ...
  microPitch: {
    ...
    build: (p) =>
      effects.microPitch({ detune: p.detune, width: p.width, delayMs: p.delay, mix: p.mix }),
  },
  rotarySpeaker: {
    label: 'Rotary',
    tag: 'ROT',
  ...
  phaserSmallStone: {
    ...
    build: (p) =>
      effects.phaser({ voicing: 'smallStone', rate: p.rate, depth: p.depth, mix: p.mix }),
  },
  uniVibe: {
    label: 'Uni-Vibe',
    tag: 'VIB',
