...
Option has 29 dual operations. The [complete dual-call
catalogue](/api/modules#complete-dual-call-catalogue) lists both shapes,
including staged currying such as `containsWith(equals)(value)(option)`.
...
fromPredicate<A, B extends A>(value: A, pred: (value: A) => value is B): Option<B>
...
```ts
map<A, B>(o: Option<A>, f: (a: A) => B): Option<B>
map<A, B>(f: (a: A) => B): (o: Option<A>) => Option<B>
flatMap<A, B>(o: Option<A>, f: (a: A) => Option<B>): Option<B>
...
tap<A>(o: Option<A>, f: (a: A) => void): Option<A>
...
```ts
getOrElse<A, B>(o: Option<A>, onNone: () => B): A | B
...
const directPort = O.fromPredicate(3000, (port: number) => port > 0 && port < 65536)
const pipedPort = pipe(
  3000,
  O.fromPredicate((port: number) => port > 0 && port < 65536),
)
