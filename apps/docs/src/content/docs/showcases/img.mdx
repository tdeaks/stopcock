...
Every parameterised filter is dual-form: `blur(img, 2)` runs data-first, while
`blur(2)` returns `(img) => Image` for use with `pipe`. The no-param ones
(`grayscale`, `sepia`, `invert`, `edgeDetect`, `equalize`) already have the
right shape.
...
const directlyBlurred = blur(img, 2)
const blurredInPipe = pipe(img, blur(2))

const processed = pipe(img, grayscale, blur(2), sharpen(1.5), brightness(20))
```
...
| `brightness(amount)`   | Adds `amount` to RGB channels. Negative darkens.                   |
| `contrast(amount)`     | Stretches or compresses values around the midpoint                 |
| `saturate(factor)`     | Multiplies saturation in HSL space. 0 = greyscale, 2 = vivid.      |
| `threshold(value)`     | Binary: pixels brighter than `value` become white, rest black      |
| `equalize(img)`        | Histogram equalization. Spreads tonal range across 0-255.          |
| `gaussianBlur(radius)` | Weighted Gaussian kernel. Softer than box blur at the same radius. |
...
const embossedDataFirst = convolve(img, embossKernel, 1)
const embossedDataLast = pipe(img, convolve(embossKernel))
```
...
resize(img, 200, 150) // data-first, bilinear interpolation
crop(img, 10, 10, 100, 80) // data-first: x, y, width, height

pipe(img, resize(200, 150), flipH, rotate90) // data-last in a pipeline
```
