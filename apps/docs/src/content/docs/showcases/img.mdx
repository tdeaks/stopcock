...
Every parameterised filter is curried: `blur(2)` returns `(img) => Image` for use with `pipe`. The no-param ones (`grayscale`, `sepia`, `invert`, `edgeDetect`, `equalize`) already have the right shape.
...
resize(200, 150)(img) // bilinear interpolation
crop(10, 10, 100, 80)(img) // x, y, width, height
...
const lines = houghLines({ threshold: 40, maxLines: 20 })(edges)
