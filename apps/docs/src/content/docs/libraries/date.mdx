...
Every signature below is dual: the data-first form on the left works, and
omitting the timestamp/leading argument returns a curried function instead
(shown here since it's the preferred, consistent-with-the-rest-of-the-docs
form). `min`, `max`, `isWeekend`, `isWeekday`, `isToday`, `isPast`, `isFuture`,
and `isValid` take one argument and aren't curried functions of anything else.

```ts
compare(a, b): number
min(dates: Timestamp[]): Timestamp        max(dates: Timestamp[]): Timestamp
clamp(lo, hi): (ts) => Timestamp
isBefore(other): (ts) => boolean          isAfter(other): (ts) => boolean
isEqual(other): (ts) => boolean           isSameDay(other): (ts) => boolean
isSameMonth(other): (ts) => boolean       isSameYear(other): (ts) => boolean
isBetween(start, end): (ts) => boolean
...
```ts
add(amount, unit): (ts) => Timestamp
subtract(amount, unit): (ts) => Timestamp
startOf(unit): (ts) => Timestamp
endOf(unit): (ts) => Timestamp
setYear(year): (ts) => Timestamp           setMonth(month): (ts) => Timestamp
setDay(day): (ts) => Timestamp             setHours(hours): (ts) => Timestamp
setMinutes(minutes): (ts) => Timestamp     setSeconds(seconds): (ts) => Timestamp
```

### Diff

```ts
diff(b, unit): (a) => number      diffInDays(b): (a) => number
diffInHours(b): (a) => number     diffInMinutes(b): (a) => number
diffInSeconds(b): (a) => number   diffInMonths(b): (a) => number
diffInYears(b): (a) => number
```

### Rounding

```ts
roundTo(unit): (ts) => Timestamp      ceilTo(unit): (ts) => Timestamp
floorTo(unit): (ts) => Timestamp      snapTo(interval, unit): (ts) => Timestamp
```
...
addDuration(d): (ts) => Timestamp     subtractDuration(d): (ts) => Timestamp
toDuration(a, b): Duration            durationToUnit(unit): (d) => number
...
```ts
range(end, step, unit): (start) => Timestamp[]
rangeBy(end, stepFn): (start) => Timestamp[]
...
addBusinessDays(amount): (ts) => Timestamp
subtractBusinessDays(amount): (ts) => Timestamp
businessDaysBetween(b): (a) => number
...
```ts
format(pattern): (ts) => string       formatter(pattern): (ts) => string
parse(pattern): (input) => Timestamp  parser(pattern): (input) => Timestamp
tryParse(pattern): (input) => Timestamp | null
