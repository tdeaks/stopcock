...
The second generic is the error response body type. When the server returns a non-2xx response, the client parses the body and throws an `HttpError<E>` with the parsed data typed.
...
interface HttpClient {
  ...
  task: TaskMethods  // same methods, returns Task<T, HttpError<E>>
