...
The core package exposes 491 dual operations. Each keeps its direct data-first
and curried data-last calls under the same name. The [complete dual-call
catalogue](/api/modules#complete-dual-call-catalogue) lists both public shapes
for every operation, including staged currying and optional arguments.

```ts
const directlyDoubled = A.map([1, 2, 3], (value) => value * 2)
const curriedDoubled = A.map((value: number) => value * 2)([1, 2, 3])
