...
const fetchAllPages = (baseUrl: string) => {
  ...
  const go = (page: number, acc: unknown[]): Task<unknown[], unknown> =>
    pipe(
      fetchPage(page),
...
const users = await Promise.all(ids.map((id) => callApi(`/users/${id}`)))
```

## Combinators

Curried, data-last, so they compose with `pipe`.

```ts
map<A, B>(f: (a: A) => B): <E>(task: Task<A, E>) => Task<B, E>
flatMap<A, B, E2>(f: (a: A) => Task<B, E2>): <E>(task: Task<A, E>) => Task<B, E | E2>
tap<A>(f: (a: A) => void | Promise<void>): <E>(task: Task<A, E>) => Task<A, E>
mapError<E, E2>(f: (e: E) => E2): <A>(task: Task<A, E>) => Task<A, E2>
catchError<E, B>(f: (e: E) => B): <A>(task: Task<A, E>) => Task<A | B, never>
flatMapError<E, B, E2>(f: (e: E) => Task<B, E2>): <A>(task: Task<A, E>) => Task<A | B, E2>
match<A, E, B, C>(handlers: { ok: (a: A) => B, err: (e: E) => C }): (task: Task<A, E>) => Task<B | C, never>
```
...
```ts
const results = await pipe(tasks, parallel(4), run)
```
...
fallback<B, E2>(backup: Task<B, E2>): <A, E>(task: Task<A, E>) => Task<A | B, E2>
```
