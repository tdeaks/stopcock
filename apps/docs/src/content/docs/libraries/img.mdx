...
Pixel-level image manipulation on raw RGBA buffers. Filters, convolution, resize, crop, the usual. Image operations that combine an `Image` with configuration are dual-form, so they work in `pipe` point-free.

Each dual pair lists the direct data-first signature before the curried data-last signature.

```ts
import { pipe } from '@stopcock/fp'
import { create, grayscale, blur, resize, brightness } from '@stopcock/img'

const image = create(800, 600)

const directlyBrightened = brightness(image, 40)
const brightenedInPipe = pipe(image, brightness(40))

const processed = pipe(image, grayscale, blur(3), brightness(40), resize(400, 300))
```
...
brightness(amount: number): (img: Image) => Image
contrast(img: Image, amount: number): Image
contrast(amount: number): (img: Image) => Image
invert(img: Image): Image
threshold(img: Image, value: number): Image
threshold(value: number): (img: Image) => Image
sepia(img: Image): Image
saturate(img: Image, factor: number): Image
saturate(factor: number): (img: Image) => Image
colorize(img: Image, target: Color): Image
colorize(target: Color): (img: Image) => Image
duotone(img: Image, dark: Color, light: Color): Image
duotone(dark: Color, light: Color): (img: Image) => Image
simulateCVD(img: Image, type: CVDType, severity?: number): Image
simulateCVD(type: CVDType, severity?: number): (img: Image) => Image
tonemapToGamut(img: Image, srcSpace: ColorSpace, targetSpace: ColorSpace): Image
tonemapToGamut(srcSpace: ColorSpace, targetSpace: ColorSpace): (img: Image) => Image
```

<Aside type="tip">
  All filter functions return a new `Image`. The original is never mutated. Chain them freely in
  `pipe`.
</Aside>

The color-aware filters use `@stopcock/color`'s packed `Float64Array` batch API internally:

- `colorize` preserves each pixel's OKLCh lightness and replaces hue/chroma with the target color.
- `duotone` maps OKLab lightness between a dark and light color.
- `simulateCVD` applies Machado color-vision-deficiency matrices once across the whole image.
- `tonemapToGamut` gamut-maps wide-gamut pixel data into a target color space.
...
convolve(kernel: number[][], divisor?: number): (img: Image) => Image
blur(img: Image, radius: number): Image
blur(radius: number): (img: Image) => Image
gaussianBlur(img: Image, radius: number, sigma?: number): Image
gaussianBlur(radius: number, sigma?: number): (img: Image) => Image
sharpen(img: Image, amount?: number): Image
sharpen(amount: number): (img: Image) => Image
...
resize(width: number, height: number): (img: Image) => Image
crop(img: Image, x: number, y: number, w: number, h: number): Image
crop(x: number, y: number, w: number, h: number): (img: Image) => Image
...
houghLines(options: HoughOptions): (img: Image) => DetectedLine[]
