...
Curried. Routes through a hub-and-spoke graph (XYZ-D65 at the center) with memoized BFS: pay the graph walk once per unique pair, O(1) after.
...
All route through OKLCh, then return to the source space. Curried.
...
hueInterpolate(h1: number, h2: number, t: number): number  // shorter-arc, not curried
```
...
contrastRatio(b: Color): (a: Color) => number     // WCAG ratio, curried
meetsAA(b: Color): (a: Color) => boolean          // ratio >= 4.5, curried
meetsAAA(b: Color): (a: Color) => boolean         // ratio >= 7, curried
meetsAALarge(b: Color): (a: Color) => boolean     // ratio >= 3, curried
deltaE(b: Color): (a: Color) => number            // CIEDE2000 perceptual distance, curried
deltaEOK(a, b): number                            // Euclidean distance in OKLab, not curried
```
...
const theme = {
  primary: toHex(brand),
  primaryHover: toHex(lighten(0.05)(brand)),
  primaryActive: toHex(darken(0.05)(brand)),
...
const onBg = (bg: Color) => (contrastRatio(bg)(rgb(1, 1, 1)) >= 4.5 ? rgb(1, 1, 1) : rgb(0, 0, 0))
```
