...
Current slice: CRITICAL_BOUNDARY_AUDITS
Last verified commit: 8fad4f3ffbd1feb29562fc481b954a1311278522
Last controller run: 2026-07-27
...
| S11R  | IN_PROGRESS | Corrective prerequisite stage authorized on 2026-07-26: repair S2/S7 compiler integrity, bind the complete S10X extracted-artifact matrix, and obtain fresh critical-boundary audits before S11. Source/test slice: static Plan IR, import-aware exact/pure lowering, deterministic whole-core receipts, hashed external locators, Rspack, five-host composition-engine gates, and packed compiler smoke validation pass. FP packed-package-contract repair also passes independently; compact `compilePure`, the extracted matrix, and fresh audits remain. The independently audited compact-pure/compiler source-and-test slice now passes with sealed digest `sha256:cfc4a407607e9b32fca93a9b38b1a8fd1343adbe0df8fc7c088d4411dfc34f90`; checkpoint application is pending. The optimizer ABI, compiler artifact-context receipts, and complete extracted-host/layout qualification harness are source-valid; this new source checkpoint is pending before a fresh cohort is packed. The first real cohort replay exposed and now has a focused repair for export-hidden transitive package manifests; checkpoint application is pending before repacking. The second replay copied the isolated closure and exposed a false plugin-shape smoke assertion; its focused harness repair is checkpoint-pending before repacking. The third replay exposed that the generated compiler receipt validator shipped only a declaration; a private packed runtime-entry repair is checkpoint-pending. The fourth replay reached real host graph auditing and exposed a macOS physical-path alias mismatch; its fail-closed canonicalization repair is checkpoint-pending. The fifth replay proved the `.mjs` consumer was outside the compiler default filter and that pre-tree-shake observation was not emitted-retention evidence; both repairs are checkpoint-pending. The sixth replay reached the real Vite source-map gate and exposed temporary extracted paths in generated maps; fail-closed canonical source identities, physical containment, and exact code-to-map linkage are checkpoint-pending. The seventh replay cleared source-map identity and exposed lexical receipt-root comparison across macOS physical aliases; strict regular-file containment and external-locator hardening are checkpoint-pending. The eighth replay cleared receipt identity and exposed a CommonJS wrapper mismatch against the frozen ESM Webpack size denominator; complete Webpack-family ESM alignment is checkpoint-pending. The ninth replay cleared the corrected Webpack size gate and exposed host-dependent logical versus physical source spellings in strict diagnostics; exact physical-identity matching is checkpoint-pending. The tenth replay recognized the exact strict rejection and exposed partial receipt publication from the failed transform; transactional receipt invalidation and atomic commit are checkpoint-pending. The eleventh replay exposed a false compact-fusion graph contract that required its zero-logic façade rather than its retained hashed execution engine; execution-engine and behavioral tier contracts are checkpoint-pending. The twelfth replay reached the compact-compile strict row and exposed a harness-invented diagnostic reason narrower than the compiler's canonical flow/compile parser reason; the shared exact diagnostic contract is checkpoint-pending. The thirteenth replay cleared all mixed tiers and exposed Unplugin 3.3 discarding compiler maps when Webpack or Rspack had no incoming loader map; package-owned high-resolution map seeding for both adapter families is checkpoint-pending. The fourteenth replay retained exact columns but exposed physical FP and consumer sources collapsed to ambiguous Webpack virtual basenames; marker-proven adapter finalization now restores physical identities without changing canonical receipt maps and is checkpoint-pending. The fifteenth replay cleared the source-map matrix and exposed an import-pruning graph contract that required a zero-byte root façade instead of the retained hashed Option engine; exact execution-engine and host-evidence contracts are checkpoint-pending. The sixteenth replay cleared import pruning and exposed the real 512-byte two-unrelated-helper gate: unary `trim` retained the generic tagged `dual` and whole opcode table. Fixed-numeric, provenance-preserving String operators now meet the unchanged packed four-host ceiling with exact legacy reflection; this repair is checkpoint-pending. The seventeenth replay cleared the packed checker and every host row and then stopped inside the packed-CLI evidence gate, which scanned the whole shared `NODE_DEBUG=esm` stream and matched the CLI's own rendered capability prose rather than any loaded module; specifier-bound closure derivation and a pid-free closure digest are checkpoint-pending. The eighteenth replay cleared that gate and exposed the same block comparing Node's physical loader spelling against a logical extracted root; physical canonicalization on both sides is checkpoint-pending. The nineteenth replay cleared the complete packed-CLI gate and reached the built-in double-materialization boundary, which reported only that the two runs differed; a first-difference reporter is checkpoint-pending so the exact disagreeing field is named instead of rediscovered by rerun. The twentieth replay named the disagreement as the first common host row's raw emitted-code identity; fail-closed portability assertions and byte counts on emitted code are checkpoint-pending so the leak is named at emission instead of inferred from a hash. The twenty-first replay named the leak exactly: Vite writes absolute `//#region` module boundaries into the bundle, so raw bundle bytes are a host-specific spelling; scratch-root canonicalization of emitted code, with the frozen gzip denominator left on the unmodified bytes, is checkpoint-pending. The twenty-second replay passed the complete extracted host matrix, including both byte-identical materializations, and reached the extracted layout matrix for the first time, where the ordinary install compared Node physical locators against logical install roots; physical install roots are checkpoint-pending. The twenty-third replay cleared the ordinary layout and exposed the deliberate ABI-mismatch mutator assuming the bundler inlines every identity, while it keeps the four hash identities as references to their generated constants; mutating whichever spelling the build emitted is checkpoint-pending. The twenty-fourth replay constructed the deliberate mismatches and exposed the mismatch-reason assertion contradicting the harness's own expectation model for the one healthy shared-pair case; conditioning that assertion on the modelled expectation is checkpoint-pending. The twenty-fifth replay passed the complete extracted host and layout qualification, Qualification A and B are byte-identical across all three components, the fresh S2/S7 replay is green apart from six proven-unrelated baseline files, and the bound qualification-evidence manifest exists; only the two fresh critical-boundary audits remain before S11R can claim its gate. Both audits then ran: S7 returned PASS, and S2 returned FAIL because the evidence-manifest script was committed inside the packaged compiler source alongside the cohort it was not packed from, so that cohort can never pass its own packed-identity check; the repair, a locale-independent comparator, and a repacked cohort are checkpoint-pending. The repacked cohort now passes its own packed-identity check at a committed tree, A and B are byte-identical, and a third run under a different collation locale reproduces the same bytes, so the evidence is measured host-independent rather than asserted; fresh audits at this new tree remain outstanding. |
...
- S11R extracted source-map canonicalization repair evidence:
  ...
  - the stale source-bound cohort was moved recoverably to
    ...
    No timing command ran and no extracted qualification result is claimed.
- S11R receipt physical-containment repair evidence:
  - fresh cohort
    `sha256:40cdf0f4c4f948ee2fe3a22863d3a1b5b7c52e8dd987b2c3097884c101a654a8`
    passed the complete 21-package checker. It cleared the compiler filter,
    emitted-contributor, and canonical source-map gates, then failed closed
    when the emitted receipt used an external locator instead of
    `src/compiler.collect.common.mjs`;
  - the configured root retained macOS's logical `/var/.../consumer` spelling
    while Vite supplied the same source through its physical
    `/private/var/.../consumer` spelling. The receipt emitter compared them
    lexically and therefore misclassified an in-root file as external;
  - receipt paths now become project-relative only for a native absolute path
    or queryless file URL whose real source is a regular file physically
    contained by a real directory root. Physical root aliases canonicalize to
    the same relative identity; an in-root symlink escaping the root remains
    external;
  - directory, missing, queried, fragmented, virtual, relative, malformed URL,
    and outside-root IDs retain opaque
    `stopcock.receipt.external-source.v1` hashes of their normalized original
    complete host ID. Distinct outside aliases remain distinct and no raw
    machine path enters a receipt;
  - compiler source/types/build validation passes with 17 files and 455 tests;
    focused receipt, plugin, and S11R tests pass 48 tests; the reproduced
    `/var` to `/private/var` probe now returns `src/entry.mjs`; diff integrity
    passes. Independent physical-alias and security audits both returned
    `PASS`;
  - the stale source-bound cohort was moved recoverably to
    `/tmp/stopcock-s11r-stale-cohort.kAoLSQ/40cdf0f4c4f948ee2fe3a22863d3a1b5b7c52e8dd987b2c3097884c101a654a8`.
    No timing command ran and no extracted qualification result is claimed.
- S11R Webpack-family size-denominator repair evidence:
  - fresh cohort
    `sha256:f89a963cd2549d0e1b915c83c77284c0c5ce7f623c31fa49f11ffcec755fc05c`
    passed the complete 21-package checker with compiler tarball
    `sha256:75867fb38a737473d1768becb24d1b6e75f51dfdbb4a7b53c4098329700ec6a5`,
    FP tarball
    `sha256:9e2f0fc3059d28e83ee876e0cbf99e5393440cf85c2d607b69f7ec8145810876`,
    and optimizer tarball
    `sha256:952fbf15c2b64562e0912a80df5a3e67695deea82cf65ed28a16d725f1bdbe4f`;
    it cleared the receipt and prior extracted gates, then failed the unchanged
    1,024-byte ceiling because `webpack/compiler.deep` measured 1,063 gzip
    bytes under the harness's CommonJS2 wrapper;
  - the retained 2,623-byte minified artifact contains the required observable
    operator-construction calls and permitted construction leaves, but no FP
    execution engine. A controlled six-format replay produced identical
    `[8,14]` behavior: Webpack measured 1,063 bytes as CommonJS2, 1,107 as
    CommonJS static, and 842 as output-module ESM; Rspack measured 1,029,
    1,073, and 966 bytes respectively;
  - the frozen S1A Webpack denominator already uses output-module ESM. The
    extracted matrix now centralizes that same `outputModule`, `module`,
    module-library, and modern-environment envelope for every Webpack and
    Rspack common, source-map, and observable-construction row plus the plain
    Webpack helper row. Each host returns its physical `out.mjs` path for
    execution and source-map verification instead of reconstructing an
    extension;
  - the 1,024-byte compiler ceiling, the 512-byte helper ceiling, production
    compiler/runtime code, frozen benchmark evidence, graph allowlist, and
    minifier settings are unchanged. Source and packed CommonJS2 adapter tests
    remain for both Webpack and Rspack, and a focused regression freezes the
    extracted ESM output profile;
  - script syntax, diff integrity, three focused host/pack/harness files with
    23 tests, and the compiler release gate with 17 files and 456 tests pass.
    Independent module-extension and size-denominator re-audits both returned
    `PASS`;
  - the stale source-bound cohort was moved recoverably to
    `/tmp/stopcock-s11r-stale-cohort.aQYzCD/f89a963cd2549d0e1b915c83c77284c0c5ce7f623c31fa49f11ffcec755fc05c`.
    No timing command ran, no size ceiling was weakened, and no extracted
    qualification result is claimed.
- S11R strict-diagnostic physical-identity repair evidence:
  - fresh cohort
    `sha256:968d99ea2054df44fa941f2b17dd46b888c1d842794f46d40d165fe397791a40`
    passed the complete 21-package checker and the corrected Webpack-family
    common behavior, graph, source-map envelope, receipt, and 1,024-byte size
    gates. The first strict mixed-tier Vite row then rejected the intended
    spread-argument `pipe` site, but the harness compared its physical
    `/private/var/...` source ID with the equivalent logical `/var/...` entry;
  - a controlled all-five-host strict probe proved this is not one universal
    spelling: Rollup reports the logical path, while esbuild, Webpack, Rspack,
    and Vite report the physical path. Every host retains the exact compiler
    site, line, and reason;
  - strict rejection now extracts exactly one complete, whitespace-delimited
    compiler diagnostic. It requires the exact `pipe` or `compile` site,
    positive line, whole-line reason, and absolute native source; the reported
    and expected sources must both be regular selected-topology files that
    canonicalize through real paths to the same `consumer/...` identity;
  - foreign same-named files, wrong site, wrong line, changed reason, glued
    marker substrings, duplicate complete diagnostics, non-files, and consumer
    escapes fail closed. The stable qualification row still records no raw
    machine path. Shared explicit control-character checks also remove the two
    pre-existing lint violations in this harness without changing source-map
    policy;
  - focused lint, script syntax, diff integrity, and the 13-test S11R boundary
    suite pass. The compiler release gate passes 17 files and 457 tests.
    Independent five-host wrapper and adversarial identity re-audits both
    returned `PASS`;
  - the stale source-bound cohort was moved recoverably to
    `/tmp/stopcock-s11r-stale-cohort.GbBVxM/968d99ea2054df44fa941f2b17dd46b888c1d842794f46d40d165fe397791a40`.
    No timing command ran and no extracted qualification result is claimed.
- S11R strict-failure receipt-transaction repair evidence:
  - fresh cohort
    `sha256:ffd1402c10138e8a92ff3420b4983e1aaf419da59f6e22a798342c453fec8222`
    passed the complete 21-package checker, corrected common-size matrix, and
    exact physical-identity strict diagnostic. Vite emitted no bundle output
    but its Unplugin `buildEnd` hook still wrote a receipt from the failed
    transform, violating the strict gate's all-or-nothing evidence rule;
  - the compiler plugin now latches a transform failure at the shared
    five-host transform seam, clears buffered receipts, invalidates the exact
    canonical receipt target, and suppresses both file and callback commit.
    `buildStart` also invalidates a prior successful document before resetting
    the watch-cycle buffer, so a failed rebuild cannot expose stale evidence;
  - successful receipt JSON writes to a unique same-directory file with
    exclusive creation, atomically renames into place, and removes its
    temporary file in all cases. The callback runs only after file commit; a
    write or callback exception invalidates the canonical target. Native
    Rollup/Vite `buildEnd(error)` also aborts the receipt transaction;
  - one plugin-instance regression covers successful publication, stale-file
    removal, a later supported transform followed by strict rejection, native
    host failure, callback counts, and the next successful rebuild. A direct
    live-source probe then proved all five real hosts reject the strict spread
    site with zero output files, zero receipt files, and zero callbacks;
  - focused source/public types, lint, script syntax, diff integrity, and the
    plugin plus S11R boundary suites pass 21 tests. The compiler release gate
    passes 17 files and 458 tests. Independent receipt-lifecycle and stale-file
    transaction audits both returned `PASS`;
  - the portable guarantee is deliberately scoped to compiler-transform
    failures plus native Rollup/Vite terminal errors. Unplugin 3.3 does not
    expose unrelated post-transform esbuild or Webpack-family failure state
    through one common hook; S11R neither claims nor requires that broader
    guarantee;
  - the stale source-bound cohort was moved recoverably to
    `/tmp/stopcock-s11r-stale-cohort.68noYG/ffd1402c10138e8a92ff3420b4983e1aaf419da59f6e22a798342c453fec8222`.
    No timing command ran and no extracted qualification result is claimed.
- S11R mixed-tier execution-contract repair evidence:
  - fresh cohort
    `sha256:750497e78e7d58ebb0e8731bc5d9f9e2a3feb2e3ce47b4ff2b411cc6153ed1dc`
    passed the complete 21-package checker, common size matrix, exact strict
    diagnostic, and zero-receipt rejection gates before Vite reported that
    compact fusion had pruned `dist/fusion.js`;
  - packed `dist/fusion.js` is a zero-logic re-export façade. Rollup-family
    emitted-byte metadata correctly omits it and retains the real hashed
    `dist/compact-runtime-*.js` execution engine. A controlled packed probe
    also confirmed esbuild omits the façade, while Webpack and Rspack retain
    both identities in their final-chunk membership graphs;
  - mixed-tier graph contracts now require the actual sequential, compact, or
    optimizer execution engine. Vite, Rollup, and esbuild additionally reject
    incompatible emitted-byte contributors. Webpack and Rspack record their
    weaker final-chunk reachability evidence without making a false
    byte-exclusion claim. A façade may be observed but can never satisfy the
    positive engine contract;
  - the mixed fallback oracle now records callback order through `map` and
    early-exit `some`. Sequential root must finish the complete map stage before
    the terminal, while compact fusion, compact compile, and optimized fusion
    must interleave per element and stop after the second input. Direct runtime
    evidence produced exactly those four tier traces;
  - script syntax, focused lint, diff integrity, and the 14-test S11R boundary
    suite pass. The compiler release gate passes source types, public types,
    and 17 files with 459 tests. An independent adversarial graph, runtime,
    strict-line, and output-stability audit returned `PASS`;
  - the source-bound cohort was moved recoverably to
    `/tmp/stopcock-s11r-stale-cohort.FYgr2t/750497e78e7d58ebb0e8731bc5d9f9e2a3feb2e3ce47b4ff2b411cc6153ed1dc`.
    No timing command ran and no extracted qualification result is claimed.
- S11R compact-compile strict-diagnostic repair evidence:
  - fresh cohort
    `sha256:3d3b38b659981ef7b2f6133afd1ac17ef7870ac472c23aecc32fa11b5c0adb9e`
    passed the complete packed checker and the repaired sequential and compact
    fusion mixed-tier rows before the compact-compile strict row rejected the
    harness expectation. The compiler reported the exact site and line but its
    canonical reason was `spread arguments in flow()/compile() call`, not the
    harness-invented `spread arguments in compile() call`;
  - one shared mixed-tier diagnostic contract now supplies the strict host
    assertion and deterministic harness-corpus identity. It records `pipe`,
    line 11, and the pipe-specific reason for pipe tiers; compact compile
    records `compile`, line 11, and the compiler's real shared flow/compile
    parser reason;
  - the source authority in `transform.ts`, an exact live compact-compile
    transform probe, and the manifest-boundary regression all agree on the
    diagnostic. Script syntax, focused lint, diff integrity, the 14-test S11R
    boundary suite, source types, public types, and the 17-file 459-test
    compiler release gate pass. Independent adversarial review returned
    `PASS`;
  - the source-bound cohort was moved recoverably to
    `/tmp/stopcock-s11r-stale-cohort.q6XuLY/3d3b38b659981ef7b2f6133afd1ac17ef7870ac472c23aecc32fa11b5c0adb9e`.
    No timing command ran and no extracted qualification result is claimed.
- S11R Webpack-family source-map preservation repair evidence:
  - fresh cohort
    `sha256:d72bd22c9234e64d675efafff63263049775e851cca4e41b622318402b20f0c4`
    passed the complete packed checker and all four mixed tiers across all five
    hosts before Webpack mapped the callback throw to line 4, column 0 instead
    of the exact original line 4, column 19;
  - the compiler's own high-resolution MagicString map resolves that token to
    column 19, and the S11R sanitizer preserves its mappings. Unplugin 3.3's
    Webpack-family loader instead discards the transform map when no incoming
    loader map exists, leaving Webpack and Rspack with only a line-level
    anchor;
  - the package-owned Webpack and Rspack adapters now append a private
    high-resolution identity-map seed loader to Unplugin's selected transform
    rule. Existing incoming maps pass through unchanged; root and named
    adapters retain the original filter, context, lifecycle, and receipt
    behavior. The wrapper fails closed if the locked rule topology changes;
  - the compiler release gate passes source types, public types, packed
    installation, and 17 files with 461 tests. The focused real-host suite
    passes 8 tests, direct built-output Webpack and Rspack probes both resolve
    exact column 19, focused lint and diff integrity pass, and independent
    adversarial review returned `PASS`;
  - the source-bound cohort was moved recoverably to
    `/tmp/stopcock-s11r-stale-cohort.6ZmqJA/d72bd22c9234e64d675efafff63263049775e851cca4e41b622318402b20f0c4`.
    No timing command ran and no extracted qualification result is claimed.
- S11R Webpack-family physical source-identity repair evidence:
  - fresh cohort
    `sha256:7ef53c78a9652eb2fdc7d85cf83dbb23336fcbd66bbfec75d389c6b385cea066`
    passed the complete packed checker, common rows, and all four mixed tiers
    across the five hosts before Webpack rejected
    `provenance-B1pK4MB-.js` as an unknown virtual source;
  - the provenance module is a physical extracted FP file. The first seed
    map's identical `file` and `source` values collapsed that identity to a
    basename, and the compiler's deliberately receipt-stable direct map has
    the same relative shape. Weakening the virtual-source allowlist or making
    canonical compiler maps scratch-absolute was rejected;
  - the Webpack-family loader now forms a two-phase bridge around Unplugin:
    every incoming or generated identity map receives a private collision-safe
    marker before the transform. No-op maps retain that marker and are restored
    byte-equivalently; only an unmarked, exact compiler-map shape may be
    rebound to the physical resource afterwards. Canonical compiler maps,
    receipt hashes, and the emitter ABI remain unchanged;
  - the compiler release gate passes source types, public types, packed
    installation, and 18 files with 463 tests. The focused adapter suite passes
    10 tests, the S11R boundary suite passes 14 tests, focused lint and diff
    integrity pass, and independent adversarial review returned `PASS`;
  - the source-bound cohort was moved recoverably to
    `/tmp/stopcock-s11r-stale-cohort.vFHhnm/7ef53c78a9652eb2fdc7d85cf83dbb23336fcbd66bbfec75d389c6b385cea066`.
    No timing command ran and no extracted qualification result is claimed.
- S11R import-pruning execution-engine contract repair evidence:
  - fresh cohort
    `sha256:c6a167ed1572049788344cc219ebee02f43a56da1f514cf02b02de210c0dbd10`
    passed the complete packed checker, common and mixed rows, and the exact
    source-map matrix before Vite reported that import pruning had lost the
    needed root sibling module;
  - the transformed source retains `some`, removes only `pipe`, retains the
    fallback `compile`, and executes the expected sibling result. Vite's
    emitted-byte graph truthfully omits zero-byte `dist/index.js` and retains
    the physical `dist/option-B35NiKCI.js` implementation with 298 rendered
    bytes;
  - the graph contract now requires the exact hashed Option implementation and
    exact `dist/compile.js` fallback. Vite, Rollup, and esbuild additionally
    require the pruned root façade to contribute no emitted bytes; Webpack and
    Rspack record only final-chunk reachability. Optimizer exclusion remains;
  - adversarial suffix identities cannot satisfy or falsely trip the exact
    contracts. Script syntax, focused lint, diff integrity, the 15-test S11R
    boundary suite, and the compiler release gate with 18 files and 464 tests
    pass. Independent adversarial review returned `PASS`;
  - the source-bound cohort was moved recoverably to
    `/tmp/stopcock-s11r-stale-cohort.J4Pwhk/c6a167ed1572049788344cc219ebee02f43a56da1f514cf02b02de210c0dbd10`.
    No timing command ran and no extracted qualification result is claimed.
- S11R fixed-numeric String-helper repair evidence:
  - fresh cohort
    `sha256:20d983dfcdb73d355a5c001eecc1edd8026b9a6a5fc868431d63c2b833efb3b1`
    passed the complete packed checker, common and mixed rows, exact source-map
    matrix, and corrected import-pruning contract before esbuild measured
    `helpers.two-unrelated` at 1,216 gzip bytes against the unchanged 512-byte
    ceiling;
  - isolated packed diagnostics proved `array.map` already passed at 464 gzip
    bytes while `string.trim` cost 854 bytes because its tagged unary
    initializer retained the generic `dual` and complete runtime opcode table.
    The failure reproduced under every required bundler and the frozen
    Terser 5.49.0 plus gzip-9 denominator; neither the fixture nor the ceiling
    was stale;
  - all seven tagged unary String operators now use a fixed numeric opcode
    path that preserves their enumerable public `_op`, registers the same
    callable in private provenance, and remains droppable when unused.
    `split` uses a fixed-numeric anonymous dispatch wrapper and registers its
    exact captured separator only on the data-last path, so it no longer pins
    the generic opcode lookup at module initialization;
  - adversarial review found and rejected a first version whose named
    functions changed public reflection. The corrected unary arrows preserve
    empty names, arity one, and non-constructability; the anonymous split
    wrapper preserves empty name, arity zero, and constructability. Regression
    tests bind those facts plus public tags, private provenance, and captured
    separator identity;
  - a fresh extracted-tarball audit executed the canonical helper fixture and
    measured esbuild 504, Rollup 501, Rolldown 501, and Webpack 509 gzip bytes.
    `string.trim` independently measures 149 gzip bytes with no opcode table.
    The 512-byte helper ceiling and frozen minifier/compression settings are
    unchanged;
  - FP codegen/build reproducibility remains
    `sha256:4136a9e0b73f9995c8ee44af5474b40b595a50e3fe05b1e3644abd12ac91e3a7`.
    The full FP release contract passes 49 files with 1,243 tests, built
    purity reports 16 generated, seven registered unary, one registered dual,
    54 manual dual, and one immutable initializer, the packed 44-module
    consumer contract passes, and the optimizer passes 10 files with 1,797
    tests. S3B's former `trim` deferral is now an enforced 717-byte/no-opcode
    gate and its five-test suite passes;
  - the canonical prose already admits focused FP implementation seams and
    size fixtures required by the extracted matrix, but the machine scope
    omitted these exact paths. Outer control-plane commit
    `cf7be54b2160904dc08b77cc5e52b4d49d97b177` admitted only the five required
    String, purity, and S3B paths; the canonical S11R scope checker and
    workspace checker now pass;
  - both final independent adversarial audits returned `PASS`. The failed
    cohort was moved recoverably to
    `/tmp/stopcock-s11r-stale-cohort.rHsDI1/20d983dfcdb73d355a5c001eecc1edd8026b9a6a5fc868431d63c2b833efb3b1`.
    No timing command ran and no extracted qualification result is claimed.
- S11R packed-CLI ESM closure evidence-scope repair:
  - source-bound cohort
    `sha256:daf5f634acaf577ea72c892cccd9a1a3cfe723d6c16e893e69225be11b79aba1`
    passed the explicit packed replay for all 21 public tarballs with FP at
    134,070 bytes, the compiler at 74,007 bytes, and the optimizer at 30,129
    bytes. Qualification A then stopped at its first exact failure inside the
    packed-CLI gate with `packed CLI ESM closure imports FP/optimizer/fusion
    runtime`;
  - the blocker is harness, not product. The gate applied its
    FP/optimizer/fusion pattern to the entire captured stderr, but the packed
    CLI writes its rendered explanation to that same stream, and the three
    matches were the semantic capability ids `@stopcock/fp/array/filter@1/exact`,
    `@stopcock/fp/array/map@1/exact`, and `@stopcock/fp/array/take@1/exact` in
    that prose. The closure Node actually recorded is two extracted compiler
    modules, `dist/cli.js` and `dist/receipt-schema.generated-BI5IapGk.js`, plus
    `node:crypto`, `node:fs`, `node:fs/promises`, `node:path`, and `node:url`.
    No FP, optimizer, or fusion module is loaded, so the canonical requirement
    that the CLI import no production fusion runtime to render evidence holds on
    the packed artifact;
  - the same block carried a second latent defect that would have failed the
    A/B boundary next: `esmClosure.debug` hashed the raw stream, which carries
    the `ESM <pid>:` prefix, so independent materializations could not have
    produced byte-identical evidence and a raw host id would have leaked into
    the output;
  - the repair adds `cliEsmClosureSpecifiers`, which derives the closure from
    the `Storing <specifier>` load records, rejects any FP, optimizer, or fusion
    specifier individually, and keeps the existing extracted-compiler
    containment check for file specifiers. The evidence now records the sorted
    compiler-relative specifier list and a digest over that canonical list
    instead of the raw stream, so no pid and no temporary extraction root
    reaches the output;
  - script syntax, the S11R boundary suite at 17 tests including a new focused
    closure test, and focused lint over both changed paths pass. The root lint
    task retains an unrelated pre-existing baseline failure in
    `apps/synth-studio`, `env.d.ts`, and `packages/fp/scripts/check-built-purity.ts`
    that predates this slice and is not absorbed by it;
  - the qualifier is bound by the cohort's own source identity, so the repair
    drifts `@stopcock/fp-compiler` and invalidates the cohort. It was moved
    recoverably to
    `/tmp/stopcock-s11r-quarantine-YSrNQa/daf5f634acaf577ea72c892cccd9a1a3cfe723d6c16e893e69225be11b79aba1`.
    A fresh cohort must be packed from this clean checkpoint before
    qualification restarts. No timing command ran and no extracted qualification
    result is claimed.
- S11R packed-CLI closure physical-identity repair:
  - fresh cohort
    `sha256:e5c7a9093a92edfa84d6a183a7557bacb5f3b5e3c48b0520e2415df828afd4e8`
    packed from the clean closure-evidence checkpoint. Its FP, compiler, and
    optimizer tarballs are byte-identical to the quarantined `daf5f634` cohort
    at 134,070, 74,007, and 30,129 bytes with unchanged SHA-256 values, which
    independently proves the previous repair changed only unpacked harness
    source. The explicit packed replay passed for all 21 public tarballs;
  - Qualification A cleared the corrected closure gate and stopped at the next
    exact failure in the same block: `packed CLI ESM closure escapes extracted
    compiler`. Node reports the loader's physical `/private/var/...` spelling
    while the extracted topology held the logical `/var/...` root, so a
    correctly contained module compared as an escape. This is the same macOS
    physical-alias class already repaired for source maps, receipt roots, and
    strict diagnostics, and it was unreachable until the prose false positive
    was removed;
  - the repair canonicalizes both sides, matching the established pattern that
    the file already uses for extracted module identity: the extracted compiler
    root and every file specifier in the closure are resolved physically before
    containment and before the compiler-relative evidence spelling is computed;
  - script syntax and the 16-test S11R boundary suite pass, and focused lint
    over the changed path is clean. The unrelated root lint baseline is
    unchanged and not absorbed;
  - the cohort was moved recoverably to
    `/tmp/stopcock-s11r-quarantine-YSrNQa/e5c7a9093a92edfa84d6a183a7557bacb5f3b5e3c48b0520e2415df828afd4e8`.
    No timing command ran and no extracted qualification result is claimed;
  - the previous entry states the S11R boundary suite at 17 tests. The measured
    count is 16. That entry is retained byte-for-byte because evidence history
    is append-only; the correction is recorded here. Only the transcribed count
    was wrong. The suite membership, the new focused closure test, and the
    passing result it describes are unchanged.

- S11R materialization-difference diagnostic:
  - fresh cohort
    `sha256:2d1fc358ad25c76a195a925adcd7e1adc4a3baaa2ab4cde1304e788afb809597`
    packed from the clean physical-identity checkpoint passed the explicit
    packed replay for all 21 public tarballs. Qualification A then cleared the
    complete packed-CLI gate, including the deterministic pass, duplicate,
    forged-core, invalid, missing, and stale rows, and reached the built-in
    double-materialization boundary;
  - that boundary failed with `two independent extracted materialisations
    produced different qualification bytes` and named no field. The canonical
    failure rule requires the exact first failure, and a bare byte verdict
    cannot be acted on without a further pack-and-replay cycle, so the missing
    diagnostic is itself the blocker to repair before the difference is
    diagnosed;
  - the repair adds `firstDifference`, which walks the two materialization
    values in sorted key order and reports the first disagreeing path with both
    truncated values, distinguishing scalar drift, array length drift, and
    missing keys. The comparison itself is unchanged, so no determinism
    requirement is weakened;
  - script syntax, the S11R boundary suite at 17 tests including a new focused
    reporter test, and focused lint over both changed paths pass. The unrelated
    root lint baseline is unchanged and not absorbed;
  - the cohort was moved recoverably to
    `/tmp/stopcock-s11r-quarantine-YSrNQa/2d1fc358ad25c76a195a925adcd7e1adc4a3baaa2ab4cde1304e788afb809597`.
    No timing command ran and no extracted qualification result is claimed.

- S11R emitted-code portability diagnostic:
  - fresh cohort
    `sha256:0d433ec47a39ad87fb92b00da89821bfce2fca64924234210faefb70fe6c289f`
    packed from the clean reporter checkpoint passed the explicit packed replay
    for all 21 public tarballs. Qualification A reached the same
    double-materialization boundary, and the new reporter named the exact
    disagreement: `common[0].code`, the raw emitted-code hash of the first
    common host row, differed between the two materializations while every
    earlier field agreed;
  - a hash cannot say what differs, and the source maps beside it are already
    proven free of absolute, workspace, and scratch paths by `rawSourceMap`, so
    the emitted code is the only unproven artifact in that row. This slice adds
    the missing proof rather than guessing at the cause;
  - `codeIdentity` now fails closed on emitted and minified code that contains a
    scratch identity, the workspace path, or the temporary root, reporting the
    surrounding text, and it records raw and minified byte counts beside the
    existing hashes so a structural difference is distinguishable from an
    equal-length substitution. Every call site passes its host and consumer
    label. The frozen gzip denominator is computed exactly as before on the
    unmodified minified code, and no comparison or ceiling is relaxed;
  - script syntax, the 17-test S11R boundary suite, and focused lint over the
    changed path pass. The unrelated root lint baseline is unchanged and not
    absorbed;
  - the cohort was moved recoverably to
    `/tmp/stopcock-s11r-quarantine-YSrNQa/0d433ec47a39ad87fb92b00da89821bfce2fca64924234210faefb70fe6c289f`.
    No timing command ran and no extracted qualification result is claimed.

- S11R emitted-code scratch-root canonicalization:
  - fresh cohort
    `sha256:c87f924a5ede1a862e4805305df226a07332a83fb3088056b03270454f92b258`
    passed the explicit packed replay for all 21 public tarballs, and the new
    emission-time assertion named the difference exactly rather than leaving a
    hash mismatch to interpret: `vite/compiler.collect.common` emits
    `//#region <absolute module path>` boundaries, so its bundle embedded
    `.../stopcock-s11r-extracted-a-Bi1WfA/extracted/fp/dist/option-B35NiKCI.js`;
  - this is Vite output, not compiler output. The transformed result, its
    receipts, and its source maps are already proven path-free. Two independent
    extractions therefore cannot produce identical raw bundle bytes by
    construction, so the raw byte hash was never a host-independent identity for
    that row;
  - the repair rewrites the scratch root to a stable `<scratch>` token before
    hashing emitted and minified code, replacing both the logical and physical
    spellings because Node and the bundlers disagree about `/var` and
    `/private/var` on macOS. This is the same canonicalization the file already
    applies to module ids, source maps, receipt roots, and the CLI closure. The
    fail-closed leak assertions now run on the canonicalized text, so any
    residual scratch, workspace, or temporary path still fails;
  - the frozen size denominator is untouched: `gzipBytes` is still computed from
    the unmodified minified bytes before canonicalization, so the 512-byte
    helper ceiling and the 1,024-byte common-consumer ceiling keep measuring
    exactly what they measured before;
  - script syntax, the S11R boundary suite at 18 tests including a new focused
    canonicalization test, and focused lint over both changed paths pass. The
    unrelated root lint baseline is unchanged and not absorbed;
  - the cohort was moved recoverably to
    `/tmp/stopcock-s11r-quarantine-YSrNQa/c87f924a5ede1a862e4805305df226a07332a83fb3088056b03270454f92b258`.
    No timing command ran and no extracted qualification result is claimed.

- S11R extracted-layout physical install roots:
  - fresh cohort
    `sha256:a417bb3a632905f248e87f094604a525493f9ad29dd622d89e661c4b0fddfd5a`
    passed the explicit packed replay for all 21 public tarballs and then passed
    the complete extracted host matrix for the first time in this stage,
    including the built-in double-materialization boundary. The scratch-root
    canonicalization holds: two independent extractions now produce
    byte-identical host evidence;
  - qualification advanced into `s11r-extracted-layouts.mjs`, which had never
    executed before, and stopped at its first exact failure: `resolved module
    escapes extracted installs` for
    `layouts/ordinary/consumer/node_modules/@stopcock/fp/dist/index.js`. The
    locator normalizer resolves each probe locator physically but compared it
    against install roots recorded with their logical spelling, so a correctly
    contained module compared as an escape. This is the same macOS
    physical-alias class already repaired for source maps, receipt roots, strict
    diagnostics, and the packed CLI closure;
  - the repair records each extracted install root physically at install time,
    so both sides of the containment comparison use the same spelling. No
    layout, mismatch, or fallback requirement is changed;
  - script syntax passes and focused lint over the changed path is clean. The
    unrelated root lint baseline is unchanged and not absorbed;
  - the cohort was moved recoverably to
    `/tmp/stopcock-s11r-quarantine-YSrNQa/a417bb3a632905f248e87f094604a525493f9ad29dd622d89e661c4b0fddfd5a`.
    No timing command ran and no extracted qualification result is claimed.

- S11R extracted-layout ABI mutation spelling:
  - fresh cohort
    `sha256:50f3f57ec373e916ec441a2c7a2ca3b6beef2e1f0f361ba667e4dcf82369159e`
    passed the explicit packed replay and the complete extracted host matrix,
    and the layout matrix advanced past install-root containment to the
    deliberate ABI-mismatch construction, where it stopped with `FP concrete ABI
    identity object must contain exactly one semanticManifestHash property
    occurrence, found 0`;
  - inspection of the packed FP `dist/abi.js` shows why. The bundler inlines the
    numeric identities, so `abiVersion: 2` and `protocolVersion: 1` appear as
    literals, but it keeps the four hash identities as references, so the frozen
    object reads `semanticManifestHash: SEMANTIC_MANIFEST_HASH` and the
    generated region holds
    `const SEMANTIC_MANIFEST_HASH = "sha256:24208c1a8b96..."`. The mutator only
    knew the inlined spelling, so it could never construct four of the six
    deliberate mismatches;
  - the repair mutates whichever spelling the build actually emitted: the
    concrete literal when the identity is inlined, otherwise the generated
    constant that the concrete object references, after asserting the object
    does reference exactly that symbol. Both paths still require exactly one
    matching site and a real byte change in the installed package, and the
    disposition records which spelling was mutated. No mismatch, fallback, or
    fail-closed requirement is weakened, and the set of constructed mismatches
    is unchanged;
  - script syntax and focused lint over the changed path pass, and both
    spellings are proved to be recognised against the real packed bundle shape.
    The unrelated root lint baseline is unchanged and not absorbed;
  - the cohort was moved recoverably to
    `/tmp/stopcock-s11r-quarantine-YSrNQa/50f3f57ec373e916ec441a2c7a2ca3b6beef2e1f0f361ba667e4dcf82369159e`.
    No timing command ran and no extracted qualification result is claimed.

- S11R extracted-layout shared-pair mismatch expectation:
  - fresh cohort
    `sha256:2c0b85a238adb243d46038f7efcfc6e93c7d4fa61355f4c6a02034f81094712a`
    passed the explicit packed replay, the complete extracted host matrix, and
    the layout install and ABI-mutation construction, then stopped at
    `abi-version has no mismatch reason`;
  - the mutation mechanism itself was proved correct against a real install
    built from these exact tarballs. Mutating the concrete inlined
    `abiVersion` yields `FP ABI version differs`, and mutating the referenced
    `SEMANTIC_MANIFEST_HASH` constant yields `FP semantic manifest hash
    differs`, so both spellings reach the negotiated field. An interim variant
    that also mutated the generated constant for the inlined numerics was
    dropped: it is unnecessary and would leave the installed package
    self-inconsistent, because the bundler inlines those numbers into
    `negotiate` as well;
  - the real blocker is the harness contradicting itself. It models one case
    where no negotiation failure is expected, the mutated secondary FP under
    `duplicate-fp-shared-optimizer`, which is a foreign duplicate rather than
    this optimizer's peer, and its runtime probe already asserts that
    disposition. The following gate then demanded a non-empty mismatch reason
    unconditionally, so the modelled healthy pair could never pass;
  - the repair conditions the mismatch-reason assertion on the same modelled
    expectation and asserts the complement, that a healthy shared pair reports
    no reason at all. Fail-closed coverage is unchanged: every case still
    asserts that no specialized runner executed and that the exact fallback
    result is unchanged, so no mismatch can silently specialize;
  - script syntax and focused lint over the changed path pass. The unrelated
    root lint baseline is unchanged and not absorbed;
  - the cohort was moved recoverably to
    `/tmp/stopcock-s11r-quarantine-YSrNQa/2c0b85a238adb243d46038f7efcfc6e93c7d4fa61355f4c6a02034f81094712a`.
    No timing command ran and no extracted qualification result is claimed.

- S11R qualification A/B green and bound:
  - cohort
    `sha256:9729c324b93df58ec5ae740db9e45dbe8e150826d37ac7ff01bce6f3ada1f3fc`
    passed the explicit packed replay for all 21 public tarballs, with FP at
    134,070 bytes
    `sha256:5371e07fd85e4588ff1c2e018add356b21e85a572f38d4afec42609ae47859ea`,
    the compiler at 74,007 bytes
    `sha256:bd98bebd81d1720976853052445a0b103823593e8db650fae20823e06562980a`,
    and the optimizer at 30,129 bytes
    `sha256:952fbf15c2b64562e0912a80df5a3e67695deea82cf65ed28a16d725f1bdbe4f`.
    Those three artifacts are byte-identical to every cohort packed during this
    repair sequence, so none of the twenty-fifth-replay repairs changed a packed
    byte;
  - Qualification A and B both passed and are byte-identical component by
    component: `combined.json`
    `sha256:55e85fcebd91d7fe91ff897d31864326bed2e6d593bdda080e6624d901dee86e`
    at 2,097 bytes, `combined.hosts.json`
    `sha256:e44e4b059a0fd9e8b92fa79c8fd7911810d66cec9c202cf7a473f78eabdd3177`
    at 226,476 bytes, and `combined.layouts.json`
    `sha256:82a94a0206565eaeb17aec3f5906d5d46943a8f1289d1f674339a1163504c4fd`
    at 648,383 bytes. With each run's own two internal materializations, the
    evidence now survives four independent extractions;
  - the required qualification-evidence manifest is implemented and written. It
    binds the cohort content hash, all six A/B paths with their SHA-256 and byte
    counts, the three qualifier script identities, and the tool versions under a
    domain-separated identity
    `sha256:75d17e8d8d0befb0032e76f1a2f3fde4c75b3ddfdfba9c94a22dc7c152a31c80`.
    Regenerating it reproduces that identity byte for byte, and it refuses to
    overwrite a differing existing identity, so the checkpointed evidence is
    immutable. It is not described as part of the cohort content hash;
  - the fresh S2/S7 replay passed `codegen:check`, FP `check:release` with
    reproducibility
    `sha256:4136a9e0b73f9995c8ee44af5474b40b595a50e3fe05b1e3644abd12ac91e3a7`
    and the 44-module packed contract, compiler `check:release` at 467 tests,
    optimizer `check:source`, `test:types` for 9 packages, and
    `build:packages`. The packed identity check was re-run after all building
    and still passes, so no source, distribution, manifest, or tarball-bound
    input drifted;
  - `test:packages` reported 5,321 passed and 34 failed across six files. Every
    one is proven to be outside S11R and none is absorbed here:
    `fuzz-correctness` contributed 27 and passes 78 of 78 in isolation on a
    quiet tree; its failures came from the parallel run racing on `dist`, and
    the same run appended 7,690 lines to the checked-in pinned corpus, which is
    the self-recording hazard this ledger already records. That write was
    preserved at
    `/tmp/stopcock-s11r-quarantine-YSrNQa/pinned-corpus.raced-run.json`
    `sha256:8e7eff32309801f3b1f3e1770d4774a66ec889f666c08cf47b5b0bfd9c784c99`
    and the corpus was restored to its committed bytes. The `fp-compiler`
    pack suite failed to collect because that same race removed
    `packages/fp/dist/index.js` mid-run, and it passes 5 of 5 in isolation. The
    `fp-interop` and `fp-testing` pack suites each assert a `^2.0.0` FP peer
    range, which contradicts this programme's own recorded decision of
    2026-07-24 to use exact internal prerelease peer ranges during the train and
    `^2.0.0` only at stable; they are owned by the release-alignment path, not
    by S11R. `s10-hand-loop-gate` contributed 4 timing rows and is quarantined
    with no verdict retained. `portable-perf-gate` failed only its pinned
    subject digest, and that subject already hashed to
    `ae7b1dd57d16007755434bffed5113f81c2f569cae375588041464abfac3851d` against
    a pin of
    `301036c1a47c4369cc0e7aa7d0c48567d137a56c6a423376367c6612371e704f` at this
    sequence's start HEAD `a97cad845f23cb8acab97c47d6e773abd58c8748`, so the
    drift predates every repair recorded here;
  - S11R remains `IN_PROGRESS`. Its exit gate additionally requires two fresh
    read-only critical-boundary audits, one S2 and one S7, at this exact tree,
    and the canonical plan forbids passing S11R on an earlier audit. No
    independent audit has been obtained for this tree, so no gate result is
    claimed. No timing command ran.

- S11R independent critical-boundary audits and their corrections:
  - two fresh read-only audits were obtained at tree
    `9d0367effb5eabdf3e4d7607b021d99a310fac73` with ledger commit
    `ca730c160eb32c12b5998ef48ca508b86609e540`. The S7 audit returned `PASS`.
    The S2 audit returned `FAIL`. S11R therefore does not pass on this cohort;
  - **correction to the previous entry.** That entry states the packed identity
    check was re-run after all building and still passes, so no
    tarball-bound input drifted. The previous entry is retained byte for byte
    because evidence history is append-only, and the correction is recorded
    here. The statement was true when the command ran and false at the commit it
    describes. `check-packed` at
    `9d0367effb5eabdf3e4d7607b021d99a310fac73` and at every later tree exits 1
    with `@stopcock/fp-compiler source drifted`. The manifest records
    `source.fileCount` 73 while the tree holds 74. The extra file is
    `packages/fp-compiler/scripts/s11r-evidence-manifest.mjs`, which was created
    after the cohort was packed and then committed in the same checkpoint as the
    cohort. Because the qualifier scripts live inside the packaged compiler
    workspace, they are bound by that package's source identity, so the cohort
    and the tree that produced it cannot coexist in one commit. The cohort was
    never valid at any committed tree. No distributed byte is affected:
    `scripts/` is not packed, the compiler tarball has no entry under it, and
    all three selected tarball digests are unchanged. The defect is confined to
    the source-provenance record, which is exactly what S2 exists to protect;
  - the stale `sha256:9729c324b93df58ec5ae740db9e45dbe8e150826d37ac7ff01bce6f3ada1f3fc`
    cohort and its A/B evidence are removed from the tree by this checkpoint
    rather than left presenting as evidence. They remain fully recoverable from
    commit `9d0367effb5eabdf3e4d7607b021d99a310fac73`. A fresh cohort is packed
    from the clean tree that already contains the evidence-manifest script, so
    the drift cannot recur;
  - **locale-dependent ordering in the identity path.** The S2 audit found that
    `compare` was `localeCompare`, which resolves against the runtime default
    locale and ICU build. Confirmed independently against the checkpointed
    evidence: 123 of 208 string arrays were in collation order rather than
    codepoint order. A different default locale or a `small-icu` runtime would
    therefore have produced different evidence bytes and a different
    qualification identity. The A/B byte-identity recorded in the previous entry
    is accordingly narrower than it reads: it established same-host
    reproducibility, not host independence. All seven comparators across the
    four qualifier scripts now use codepoint order, which removes the
    environment-dependent ordering that the canonical checklist forbids;
  - **dead tool binding.** Both audits found that the evidence manifest read
    `combined.hosts?.qualificationTools`, a path that does not exist in
    `combined.json`, so `tools.qualification` was always `null` and the eight
    bundler and minifier versions were bound only transitively. It now reads
    `qualificationTools` from the host evidence and fails closed when absent.
    The manifest additionally binds the cohort by its manifest bytes and
    cross-checks that against the hash the qualification evidence names;
  - **contingent size denominator made structural.** Both audits noted that
    `gzipBytes` is host-independent only while minification leaves no host path
    behind, which currently holds because the frozen minifier options drop
    comments. That is now asserted rather than assumed: the canonicalized
    minified text must equal the minified text. The denominator itself is
    unchanged and still computed from the unmodified minified bytes;
  - **temporary-path probe symmetry.** The S7 audit found the leak probe used
    only the logical `tmpdir()` spelling while macOS resolves it physically.
    Both spellings are now probed;
  - **byte-count naming.** The S7 audit found `codeBytes` reported a
    post-canonicalization length under a name implying the emitted length. The
    canonicalized field is renamed `canonicalCodeBytes`, and
    `minifiedCodeBytes` now reports the true minified length, which is the value
    the size ceilings are computed from;
  - findings recorded and **not** repaired here, because they belong to owners
    outside S11R and changing them would invalidate S2 evidence rather than
    requalify it: receipt-core identity is a bare `sha256(JSON.stringify(core))`
    with no domain tag and no canonical serialization, while the codebase
    already defines both patterns elsewhere; nested receipt members are copied
    by reference so a reformatted-but-equivalent receipt validates and is then
    rejected as forged; `boundaryHash` is not domain-separated and omits
    `schemaVersion` and `kind`; staleness is evaded by omitting an expectations
    line rather than by forging a hash, because absent per-site expectations are
    silently skipped; `packages/fp/codegen/generated/iter-typed-array-kernel-manifest-v1.json`
    is outside `GENERATED_PATHS` and so outside the two-generation identity; and
    `assertCleanInputGeneration` asserts only that files were created, never
    that their content matches. Each needs a canonical decision by its owning
    stage;
  - scope facts recorded so a future reader does not over-read the evidence: the
    helper matrix runs `esbuild`, `rollup`, `webpack`, and `rolldown` only, so
    `vite` and `rspack` have no `helpers.two-unrelated` row; and the seven
    `duplicate-fp-shared-optimizer/secondary/fp` mismatch rows are
    observationally equivalent to the unmutated baseline, because the mutated
    secondary FP is not the optimizer's peer. Detection of every one of those
    mismatch kinds remains proven at the other six layout and side positions and
    in all seven instrumented-gate rows, so coverage is not lost;
  - the S7 audit independently recomputed the cohort content hash, all 21
    tarball digests, the qualification identity, and the boundary hash, and
    confirmed the packed CLI is pure ESM with no `require`, no `createRequire`,
    and no dynamic import, which closes the CommonJS blind spot in the closure
    check for this artifact. Neither audit could reproduce the checkpoint
    content digest, because the checkpoint request path list is not persisted in
    the repository; both correctly reported that as unverified rather than
    wrong. No timing command ran.

- S11R requalification after the audit corrections:
  - cohort
    `sha256:cb40b58ba482a7fd56e40e70f4ca3cba5a5510ddac34936ad00538e933ffd624`
    was packed from the clean corrected tree, which already contains the
    evidence-manifest script. Its `@stopcock/fp-compiler` source record is now
    `fileCount` 74 and `check-packed` exits 0 at a committed tree, which closes
    the blocking S2 defect. The FP, compiler, and optimizer tarballs remain
    134,070, 74,007, and 30,129 bytes with unchanged SHA-256 values, so across
    every cohort in this stage no harness repair has changed a packed byte;
  - Qualification A and B both pass and are byte-identical component by
    component: `combined.json`
    `sha256:026010c968e9c...` at 2,097 bytes, `combined.hosts.json` at 226,692
    bytes, and `combined.layouts.json` at 648,383 bytes. Exact digests are
    recorded in the bound evidence manifest;
  - **locale independence is now measured, not claimed.** A third full
    qualification was run with `LC_ALL` and `LANG` set to `tr_TR.UTF-8`, a
    collation that demonstrably reorders `i`, `ı`, and `z` relative to codepoint
    order. All three components are byte-identical to the `en-US` run. With each
    run performing two internal materializations, the evidence now survives six
    independent extractions across two locales. That run was written outside the
    cohort so it cannot be mistaken for a checkpointed A/B component;
  - the qualification-evidence identity is
    `sha256:f80e364cfa055c7345f2d190d2f8c37e59c42b7be82d9ebfad83c2215cdb28a8`.
    It now binds the cohort manifest bytes as
    `sha256:28fc09117a6e3129dd29eca3c4ab05c6ab0c710c3ab02642a61875e97ac6b32d`
    in addition to the content hash, and binds all eight qualification tools
    directly with their manifest and entry digests: trace-mapping 0.3.31,
    Rspack 2.1.5, esbuild 0.28.1, Rolldown 1.0.1, Rollup 4.62.2, Terser 5.49.0,
    Vite 0.2.5, and Webpack 5.108.4. Regeneration reproduces the identity byte
    for byte and refuses to overwrite a differing one;
  - S11R remains `IN_PROGRESS`. The two audits already obtained were performed
    at tree `9d0367effb5eabdf3e4d7607b021d99a310fac73`, which this correction
    supersedes. The canonical plan forbids passing S11R on an earlier audit, so
    two fresh audits are required at the final corrected tree before any gate
    result is claimed. No timing command ran.
...
- Decision: Give tagged String helpers fixed numeric provenance initializers
  while preserving every existing callable-reflection contract, and admit only
  their exact implementation, purity-policy, test, and size-gate paths to the
  S11R machine scope.
  Rationale: The generic public `dual` initializer retained the entire opcode
  table in a one-helper bundle. A dedicated initializer removes that unrelated
  graph without weakening the 512-byte extracted gate, public tags, private
  provenance, construction semantics, or data-first/data-last behavior. The
  canonical S11R prose already owns the focused FP seams needed by its extracted
  matrix, so the machine policy needed a narrow consistency repair rather than
  a wider stage.
  Date: 2026-07-27.

## Current blockers

There is no external blocker to S11R's deterministic work. The fixed-numeric
String-helper repair that clears the unchanged extracted size ceiling is
checkpoint-pending. After that checkpoint, a clean 21-public-package cohort,
the complete extracted host/layout matrix, and fresh independent S2/S7 audits
remain. New timing evidence remains unavailable until the current host
requalifies, but timing is outside S11R and is not part of those deterministic
prerequisites.
